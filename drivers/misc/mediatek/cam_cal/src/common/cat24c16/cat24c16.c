...
static int rear_1M_inf=0;
static int rear_inf=0;
static int rear_macro=0;
int front_inf=0;
int front_macro=0;

static int rear_done=0;
static int front_done=0;

static int node_created=0;

#define IMX219_FRONT_INF_ADDR 0x11
#define IMX219_FRONT_MACRO_ADDR 0x13
#define REAR_INF_ADDR 0x84
#define REAR_1M_INF_ADDR 0x86
#define REAR_MACRO_ADDR 0x88
#define AF_FIELD_SIZE 1
#define IMX300_REAR_ADDR 0xA0
#define IMX219_FRONT_ADDR 0xA0

extern int g_camera_sensor_id;
void read_af_eeprom(void)
{
	u8 temp_result = 0;
	switch(g_camera_sensor_id)
	{
		case IMX300_SENSOR_ID:
			if (!rear_done)
			{
				selective_read_region(REAR_INF_ADDR, &temp_result, IMX300_REAR_ADDR >> 1, AF_FIELD_SIZE);
				rear_inf += temp_result * 256;

				selective_read_region(REAR_INF_ADDR + 1, &temp_result, IMX300_REAR_ADDR >> 1, AF_FIELD_SIZE);
				rear_inf += temp_result;

				selective_read_region(REAR_1M_INF_ADDR, &temp_result, IMX300_REAR_ADDR >> 1, AF_FIELD_SIZE);
				rear_1M_inf += temp_result * 256;

				selective_read_region(REAR_1M_INF_ADDR + 1, &temp_result, IMX300_REAR_ADDR >> 1, AF_FIELD_SIZE);
				rear_1M_inf += temp_result;

				selective_read_region(REAR_MACRO_ADDR, &temp_result, IMX300_REAR_ADDR >> 1, AF_FIELD_SIZE);
				rear_macro += temp_result * 256;

				selective_read_region(REAR_MACRO_ADDR + 1, &temp_result, IMX300_REAR_ADDR >> 1, AF_FIELD_SIZE);
				rear_macro += temp_result;

				rear_done = 1;
			}
			break;
		case IMX219_SENSOR_ID:
		case IMX219_CHICONY_SENSOR_ID:
			if (!front_done)
			{
				selective_read_region(IMX219_FRONT_INF_ADDR, &temp_result, IMX219_FRONT_ADDR >> 1, AF_FIELD_SIZE);
				front_inf += temp_result;

				selective_read_region(IMX219_FRONT_INF_ADDR + 1, &temp_result, IMX219_FRONT_ADDR >> 1, AF_FIELD_SIZE);
				front_inf += temp_result * 256;

				selective_read_region(IMX219_FRONT_MACRO_ADDR, &temp_result, IMX219_FRONT_ADDR >> 1, AF_FIELD_SIZE);
				front_macro += temp_result;

				selective_read_region(IMX219_FRONT_MACRO_ADDR + 1, &temp_result, IMX219_FRONT_ADDR >> 1, AF_FIELD_SIZE);
				front_macro += temp_result * 256;
...
static bool selective_read_byte(u32 addr, u8 *data, u16 i2c_id)
{
	...
	if (iReadRegI2C(&offset, 1, (u8 *)data, 1, (CATC24C16_DEVICE_ID >> 1) + page) < 0) {
...
static ssize_t show_rearcam_1M_inf(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", rear_1M_inf);
}

static ssize_t show_rearcam_inf(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", rear_inf);
}

static ssize_t show_rearcam_macro(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", rear_macro);
}

static ssize_t show_frontcam_inf(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", front_inf);
}

static ssize_t show_frontcam_macro(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", front_macro);
}

static DEVICE_ATTR(rear_1M_inf, 0664, show_rearcam_1M_inf, NULL);
static DEVICE_ATTR(rear_macro, 0664, show_rearcam_macro, NULL);
static DEVICE_ATTR(rear_inf, 0664, show_rearcam_inf, NULL);
static DEVICE_ATTR(front_macro, 0664, show_frontcam_macro, NULL);
static DEVICE_ATTR(front_inf, 0664, show_frontcam_inf, NULL);
...
void create_node(void)
{
	...
	if(node_created==0)
	{
		...
		err = sysfs_create_file(kobject_cam, &dev_attr_rear_1M_inf.attr);
		err = sysfs_create_file(kobject_cam, &dev_attr_rear_inf.attr);
		err = sysfs_create_file(kobject_cam, &dev_attr_rear_macro.attr);
		err = sysfs_create_file(kobject_cam, &dev_attr_front_inf.attr);
		err = sysfs_create_file(kobject_cam, &dev_attr_front_macro.attr);
		node_created=1;
	}
}
	...
	unsigned char *data, unsigned int size)
{
	...
	read_af_eeprom();
	ret = selective_read_region(addr, data, g_pstI2Cclient->addr, size);
	if (ret == 0 || ret != size)
	{
		CAM_CALDB("cat24c16_selective_read_region worng!\n");
		return 0;
	}
	else
	{
