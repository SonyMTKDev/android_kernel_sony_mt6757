...
void *rdtags_io_base_export;
static DEFINE_MUTEX(rdlock);
static uint8_t rdtags_initialized;
static char *events_log_buf;
static size_t events_log_size;
...
EXPORT_SYMBOL(rdtags_io_base_export);
...
static int procfs_create_node(char *name)
{
	...
	if (!entry)
		return -ENXIO;
	//for non-root user to r/w
	subentry = proc_create_data(name, S_IFREG | S_IRUGO | S_IRUGO,
					entry, &tag_fops, (void *)name);
	if (!subentry)
		return -ENOMEM;
...
static ssize_t read_events_logs(struct file *file , char __user *ubuf,
		size_t len, loff_t *offset)
{
	loff_t pos = *offset;
	ssize_t  count;

	count = MIN(len, events_log_size - pos);
	if (copy_to_user(ubuf, events_log_buf + pos, count))
		return -EFAULT;
...
static const struct file_operations events_logs_fops = {
	.read	= read_events_logs,
};

static int export_events_logs(void)
{
	int i = 0, ret = 0;
	uint8_t name[RDTAGS_NAME_SIZE];
	size_t evnt_sz[MAX_NUM_EVENTS_LOG];
	size_t pos = 0;

	dev_dbg(dev, "%s IN\n", __func__);
	while (i < MAX_NUM_EVENTS_LOG) {
		snprintf(name, sizeof(name), "event_history-%d", i);
		evnt_sz[i] = 0;
		ret = rdtags_get_tag_data(name, NULL,
				&(evnt_sz[i]));
		if (ret != -ENOBUFS)
			break;

		events_log_size += evnt_sz[i];
		dev_dbg(dev, "Read %zd bytes of data from %s\n",
				evnt_sz[i],
				name);
		i++;
	}
	...
	events_log_buf = vzalloc(events_log_size);
	if (!events_log_buf)
		return -ENOMEM;

	dev_dbg(dev, "Size of events logs is %zd\n", events_log_size);

	i = 0;
	while (i < MAX_NUM_EVENTS_LOG) {
		snprintf(name, sizeof(name), "event_history-%d", i);
		ret = rdtags_get_tag_data(name,
				(events_log_buf + pos),
				&(evnt_sz[i]));
		if (ret)
			break;
	...
	if (!proc_create("event_history", S_IFREG | S_IRUGO | S_IWUSR,
		entry, &events_logs_fops))
		dev_err(dev, "Failed to create /proc/event_history\n");
...
static int rdtags_driver_probe(struct platform_device *pdev)
{
	...
	rdtags_io_base_export = rdtags_io_base;
...
//	{
//		dev_info(dev, "wmh: normal boot ramdump_mode = %d\n", platform_data->ramdump_mode);
...
//	}
	...
	entry = proc_mkdir_mode(RDTAGS_PROC_DIR_NAME,
			S_IRWXU | S_IRGRP | S_IXGRP | S_IROTH | S_IXOTH, NULL);
