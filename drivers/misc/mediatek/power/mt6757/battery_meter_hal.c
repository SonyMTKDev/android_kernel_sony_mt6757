...
//CEI comment start
	bm_print(BM_LOG_CRTI, "[fgauge_read_current_sign] g_fg_is_charging=%d\n",g_fg_is_charging);
//sync with Android N
//CEI comment end
	return STATUS_OK;
}
...
//CEI comments, Battery ID S
extern int IMM_GetOneChannelValue(int dwChannel, int data[4], int *rawdata);
static signed int read_adc_v_bat_id(void *data)
{
	int adc_data[4];
	int adc_Volt = 0;
	IMM_GetOneChannelValue(0x4, adc_data, NULL);
	adc_Volt = adc_data[0]*1000 + adc_data[2];
	*(signed int*)(data) = adc_Volt;
	bm_print(BM_LOG_CRTI, "[read_adc_v_bat_id] adc_Volt = %d\r\n", adc_Volt);

	return STATUS_OK;
}
//CEI comments, Battery ID E
