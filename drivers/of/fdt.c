...
void __init __weak early_init_dt_add_memory_arch(u64 base, u64 size)
{
	...
	if( (base < (u64)CCI_KLOG_START_ADDR_PHYSICAL) && 
		((base + size) > ((u64)CCI_KLOG_START_ADDR_PHYSICAL + (u64)CCI_KLOG_SIZE)) )
	{
		ret = memblock_add(base, (u64)CCI_KLOG_START_ADDR_PHYSICAL - base);

		printk("[klog]-%s():start=0x%llX, size=0x%llX ret=%d\n", __func__, base, ((u64)CCI_KLOG_START_ADDR_PHYSICAL - base), ret);
		...
		if(ret == 0)
		{
			...
			printk("[klog]-%s():start=0x%llX, size=0x%llX ret=%d\n", __func__, ((u64)CCI_KLOG_START_ADDR_PHYSICAL + (u64)CCI_KLOG_SIZE), (base + size - (u64)CCI_KLOG_START_ADDR_PHYSICAL - (u64)CCI_KLOG_SIZE), ret);
		}
	}
	else
	{
