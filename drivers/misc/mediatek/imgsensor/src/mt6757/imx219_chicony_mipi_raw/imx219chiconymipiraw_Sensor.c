...
#include "imx219chiconymipiraw_Sensor.h"

/****************************Modify following Strings for debug****************************/
#define PFX "imx219chicony_camera_sensor"
...
static imgsensor_info_struct imgsensor_info = {
	.sensor_id = IMX219_CHICONY_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
#define IMX219CHICONYMIPI_MaxGainIndex (97)
kal_uint16 IMX219CHICONYMIPI_sensorGainMapping[IMX219CHICONYMIPI_MaxGainIndex][2] ={
{ 64 ,0  },
{ 68 ,12 },
{ 71 ,23 },
{ 74 ,33 },
...
{
	kal_uint8 iI;
    LOG_INF("[IMX219MIPI]enter IMX219CHICONYMIPIGain2Reg function\n");
    for (iI = 0; iI < (IMX219CHICONYMIPI_MaxGainIndex-1); iI++)
	{

        if(gain <IMX219CHICONYMIPI_sensorGainMapping[iI][0])
		{
            break;
        }
		if(gain == IMX219CHICONYMIPI_sensorGainMapping[iI][0]) //20140718 Jeff mtk07693 change from < to ==
		{
			return IMX219CHICONYMIPI_sensorGainMapping[iI][1];
		}
    ...
    if(gain != IMX219CHICONYMIPI_sensorGainMapping[iI][0])
    {
         LOG_INF("Gain mapping don't correctly:%d %d \n", gain, IMX219CHICONYMIPI_sensorGainMapping[iI][0]);
    }
	LOG_INF("exit IMX219CHICONYMIPIGain2Reg function\n");
    return IMX219CHICONYMIPI_sensorGainMapping[iI-1][1];  //here,if gain < 64, stack overflow happen.
	//return NONE;
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				...
				CAM_8M_ID = gpio_get_value(GPIO_CAMERA_8M_ID_PIN);
				if (CAM_8M_ID != 0)
				{
...
UINT32 IMX219_CHICONY_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
	/* To Do : Check Sensor status here */
	if (pfFunc!=NULL)
		*pfFunc=&sensor_func;
	return ERROR_NONE;
}	/*	IMX219_CHICONY_MIPI_RAW_SensorInit	*/
