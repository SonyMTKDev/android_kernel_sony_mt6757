...
static int16_t CTP_ID_g = -1; //20170405
extern void feedback_tpid(int16_t CTP_ID_g);
#define TPID_GPIO 54
...
*/
		...
		if (finger_cnt == 0) {
			...
			input_mt_sync(ts->input_dev);
			NVT_LOG("Touch finger up last point x=%d,y=%d\n",input_x,input_y);
		}
...
20170118 mask */
	...
	//---check input device---
	/*
	*if mtk_tpd already allocated device ,use it ,otherwise create by self
	*/
	if (tpd->dev == NULL) {
	...
	/*
	*20170616 mark ,if use mtk_tpd allocated device ,
	*for touchfilter lib that input device name need keep mtk_tpd
	*and need codework with tool team for T2 Hand crossed test
	*/
	...
	//20170405 add
	NVT_LOG("read CTP_ID start\n");
	CTP_ID_g = __gpio_get_value(TPID_GPIO);
	NVT_LOG("CTP_ID_g = %d\n",CTP_ID_g);
	feedback_tpid(CTP_ID_g);
	NVT_LOG("read CTP_ID end\n");
...
static const struct of_device_id nvt_match_table[] = {
	{.compatible = "mediatek,cap_touch_sm11l"},
	{},
};
#endif
...
static int32_t __init nvt_driver_init(void)
{
	...
	NVT_LOG("start\n");
	/* detect project id */
	NVT_LOG("customer_project_id = %d\n", get_cei_customer_project_id());
	if (get_cei_customer_project_id() != CUSTOMER_PROJECT_SM11L){
		NVT_ERR("[Optical]It's not SM11L\n");
		return 0;	
       }
