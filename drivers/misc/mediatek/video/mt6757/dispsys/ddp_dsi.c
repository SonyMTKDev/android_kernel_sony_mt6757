...
//S : LCM driver - Initial version in Android O : {
/*
...
//E : LCM driver - Initial version in Android O : }
...
//#endif
//E : LCM driver - Initial version in Android O : }
long lcm_set_tp_reset_pin(unsigned int value)
{
	long ret = 0;
#if !defined(CONFIG_MTK_LEGACY)
	if (value)
		ret = disp_dts_gpio_select_state(DTS_GPIO_STATE_TP_RST_OUT1);
	else
		ret = disp_dts_gpio_select_state(DTS_GPIO_STATE_TP_RST_OUT0);
#endif
	return ret;
}
...
int ddp_dsi_set_lcm_utils(enum DISP_MODULE_ENUM module, LCM_DRIVER *lcm_drv)
{
	...
	//E : LCM driver - Initial version in Android O : }
	utils->set_gpio_tp_reset = lcm_set_tp_reset_pin;
#endif
#endif
