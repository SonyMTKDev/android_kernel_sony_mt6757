		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	printk("%s\n", __func__);
	if (unlikely(!spi)) {
		printk("spi device is invalid\n");
		goto out;
	}
	if (unlikely(!buf)) {
		printk("buf is invalid\n");
		goto out;
	}
	
	if (!strncmp(buf, "1", strlen("1"))) {
		printk("[FPC spi] clock enable\n");
		mt_spi_enable_clk(spi);
	} else if (!strncmp(buf, "0", strlen("0"))) {
		printk("[FPC spi] clock disable\n");
		mt_spi_disable_clk(spi);
	}
	return count;
...
static DEVICE_ATTR(clk_enable, 0664, NULL, clk_enable_set);
#endif
//end
...
static struct device_attribute *spi_attribute[] = {
	...
	&dev_attr_spi_msg,
#ifdef CONFIG_INPUT_FPC1020_IRQ
	&dev_attr_clk_enable	 //Cain added for Fingerprints
#endif
};
