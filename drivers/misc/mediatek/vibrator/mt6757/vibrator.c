...
void vibr_power_set(void)
{
	...
	if (hw != NULL) {
		...
		// Grace add for noise tuning
		//pmic_set_register_value(PMIC_RG_VIBR_CAL, 0x4);
...
{
	VIB_DEBUG("%s: vosel = 0x%x , cal = 0x%x\n", __func__, vosel, cal);
	pmic_set_register_value(PMIC_RG_VIBR_VOSEL, vosel);
	pmic_set_register_value(PMIC_RG_VIBR_CAL, cal);
}
void vibr_vosel_cal_get(unsigned int *vosel, unsigned int *cal)
{
	*vosel = pmic_get_register_value(PMIC_RG_VIBR_VOSEL);
	*cal = pmic_get_register_value(PMIC_RG_VIBR_CAL);
	VIB_DEBUG("%s: vosel = 0x%x , cal = 0x%x\n", __func__, *vosel, *cal);
}
/**/
