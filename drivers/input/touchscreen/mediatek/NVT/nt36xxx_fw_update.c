...
/* 20170214 add fw_state */
extern void feedback_state(uint16_t fw_state);
static uint16_t fw_state = 0;
...
void Boot_Update_Firmware(struct work_struct *work)
{
	...
	fw_state = 1; /* 20170214 add fw_state */
	ret = update_firmware_request(firmware_name);
	if (ret) {
		NVT_ERR("update_firmware_request failed. (%d)\n", ret);
		fw_state = 2; /* 20170214 add fw_state */
		return;
	}
	...
	fw_state = 3; /* 20170214 add fw_state */
	feedback_state(fw_state);
