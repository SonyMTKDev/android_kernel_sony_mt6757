...
int wusb3801q_init_done = 0;
enum typec_current_mode wusb3801q_current_mode_get(void)
{
	...
	rc = i2c_smbus_read_byte_data(wusb_chip->client, WUSB3801_REG_STATUS);
	if (IS_ERR_VALUE(rc)) {
		pr_err("%s  fail to read register, rc=%d\n", __func__, rc);

		return TYPEC_CURRENT_MODE_DEFAULT;
	}

	bc_lvl = rc & WUSB3801_BCLVL_MASK;
	current_mode = (bc_lvl == WUSB3801_SNK_3000MA ? TYPEC_CURRENT_MODE_HIGH :
			(bc_lvl == WUSB3801_SNK_1500MA ? TYPEC_CURRENT_MODE_MID : TYPEC_CURRENT_MODE_DEFAULT));
