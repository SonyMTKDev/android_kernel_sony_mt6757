...
static int EEPROM_get_cmd_info(unsigned int sensorID, struct stCAM_CAL_CMD_INFO_STRUCT *cmdInfo)
{
	...
	if (pCamCalList != NULL && pCamCalFunc != NULL) {
		PK_DBG("EEPROM_get_cmd_info pCamCalList!=NULL && pCamCalFunc!= NULL\n");
		for (i = 0; pCamCalList[i].sensorID != 0; i++) {
			if (pCamCalList[i].sensorID == sensorID) {
				PK_DBG("EEPROM_get_cmd_info pCamCalList[%d].sensorID==%x\n", i, pCamCalList[i].sensorID);
				if(pCamCalList[i].sensorID == IMX300_SENSOR_ID)
				{
					g_camera_sensor_id = IMX300_SENSOR_ID;
				}
				else if(pCamCalList[i].sensorID == IMX219_SENSOR_ID)
				{
					g_camera_sensor_id = IMX219_SENSOR_ID;
				}
				else if(pCamCalList[i].sensorID == IMX219_CHICONY_SENSOR_ID)
				{
					g_camera_sensor_id = IMX219_CHICONY_SENSOR_ID;
				}
				else if(pCamCalList[i].sensorID == IMX234_SENSOR_ID)
				{
					g_camera_sensor_id = IMX234_SENSOR_ID;
				}
				...
				PK_DBG("EEPROM_get_cmd_info pCamCalList[%d].sensorID==%x\n", i, pCamCalList[i].sensorID);
				PK_DBG("EEPROM_get_cmd_info g_Current_Client->addr =0x%x\n", g_Current_Client->addr);
				PK_DBG("EEPROM_get_cmd_info 20 client=%p client2=%p client3=%p Cur=%p\n", g_pstI2Cclient, g_pstI2Cclient2, g_pstI2Cclient3, g_Current_Client);

				for (j = 0; pCamCalFunc[j].cmdType != CMD_NONE; j++) {
						PK_DBG("EEPROM_get_cmd_info pCamCalFunc[%d].cmdType = %d,pCamCalList[%d].cmdType = %d\n", j, pCamCalFunc[j].cmdType, i, pCamCalList[i].cmdType);
					if (pCamCalFunc[j].cmdType == pCamCalList[i].cmdType || pCamCalList[i].cmdType == CMD_AUTO) {
						if (pCamCalList[i].checkFunc != NULL) {
							if (pCamCalList[i].checkFunc(cmdInfo->client, pCamCalFunc[j].readCamCalData)) {
								PK_DBG("EEPROM_get_cmd_info pCamCalList[%d].checkFunc ok!\n", i);
	...
	unsigned int deviceID)
{
	...
	for (i = 0; i < CAM_CAL_I2C_MAX_SENSOR; i++) {
		if (g_camCalDrvInfo[i].deviceID == deviceID){
			...
			g_camera_sensor_id = g_camCalDrvInfo[i].sensorID;
			break;
	}
	}
...
#endif
	PK_DBG("EEPROM_drv_ioctl %u", a_u4Command);
