...
static ssize_t af_hall_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	int curdet = 0, hall = 0, curpos = 0;

	if (g_s4AF_Opened == 2)
	{
#ifdef CONFIG_MTK_LENS_BU64747GWZAF_SUPPORT
		curdet = BU64747GWZAF_getCurDet();
		hall = BU64747GWZAF_getHall();
		curpos = BU64747GWZAF_getCurPos();
#endif
	}
...
static ssize_t af_hall_store(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	return size;
}
static DEVICE_ATTR(hall, 0644, af_hall_show, af_hall_store);
...
static inline int Register_AF_CharDrv(void)
{
	...
	device_create_file(lens_device, &dev_attr_hall);
