...
static enum hrtimer_restart rt9466_timer_callback(struct hrtimer *timer)
{
	struct rt9466_info *info = container_of(timer, struct rt9466_info,
		timer);
...
//CEI comment start
//Add 500ms thread to prevent backboost
static int rt9466_get_charging_status(struct rt9466_info *info, enum rt9466_charging_status *chg_stat);
static inline int rt9466_check_backboost(struct rt9466_info *info)
{
	...
	if (ret < 0) {
		battery_log(BAT_LOG_CRTI, "%s: get charging status fail\n", __func__);
		return ret;
	}

	dev_info(info->dev, "%s: CHG_STATUS = %s\n", __func__, rt9466_chg_status_name[chg_status]);
	...
	if (rt9466_i2c_test_bit(info, RT9466_REG_CHG_STATC,
		RT9466_SHIFT_CHG_MIVR, &mivr_act) <= 0) {
		battery_log(BAT_LOG_CRTI, "%s: mivr loop is not act\n",
			__func__);
		return 0;
	}

	battery_log(BAT_LOG_CRTI, "%s: mivr loop is act\n", __func__);

	/* Check IBUS ADC */
	ret = rt9466_get_adc(info, RT9466_ADC_IBUS, &adc_ibus);
	if (ret < 0) {
		battery_log(BAT_LOG_CRTI, "%s: get ibus fail\n", __func__);
		return ret;
	}
	...
	if (ret < 0)
		battery_log(BAT_LOG_CRTI, "%s: toggle CFO fail(%d)\n", __func__,
			ret);

	return 0;
}

static int rt9466_thread_handler(void *data)
{
	struct rt9466_info *info = data;

	while (!kthread_should_stop()) {
		rt9466_check_backboost(info);
	...
	void *data)
{
	...
	while (adc_temp >= 120 && retry_cnt > 0) {
		dev_err(info->dev, "%s: [WARNING] t = %d\n", __func__, adc_temp);
...
//parameter setting
	...
	ret = __rt9466_set_mivr(info, 4500000);
//CEI comment end
	if (ret < 0)
...
//parameter setting
	...
	ret = rt9466_set_ieoc(info, 150000);
//CEI comment end
	if (ret < 0)
...
//parameter setting
	...
	ret = rt9466_enable_te(info, true);
//CEI comment end
	if (ret < 0)
...
//parameter setting
	...
	ret = __rt9466_enable_safety_timer(info, false);
//CEI comment end
	if (ret < 0)
...
static int rt_set_default_dpm(struct mtk_charger_info *mchr_info, void *data)
{
	int ret = 0;
	struct rt9466_info *info = (struct rt9466_info *)mchr_info;
	u32 mivr = *((u32 *)data);

	ret = __rt9466_set_mivr(info, mivr);
...
//CEI comments start, identify which charegr IC is using
static int rt_who_am_i(struct mtk_charger_info *mchr_info, void *data)
{
	return rt9466_whoami;
}
//CEI comments end, identify which charegr IC is using

//CEI comment start, SW workaround for  IEOC accuracy
static int rt_set_ieoc(struct mtk_charger_info *mchr_info, void *data)
{
	int ret = 0;
	u32 ieoc = 0;
	struct rt9466_info *info = (struct rt9466_info *)mchr_info;
...
//CEI comment start
//Add 500ms thread to prevent backboost
static int rt_charger_plug_out(struct mtk_charger_info *mchr_info, void *data)
{
	struct rt9466_info *info = (struct rt9466_info *)mchr_info;
...
static int rt_charger_plug_in(struct mtk_charger_info *mchr_info, void *data)
{
	struct rt9466_info *info = (struct rt9466_info *)mchr_info;
	ktime_t ktime = ktime_set(0, MS_TO_NS(500));
...
static int rt_charger_check_backboost(struct mtk_charger_info *mchr_info, void *data)
{
	int ret = 0;
	struct rt9466_info *info = (struct rt9466_info *)mchr_info;
	ktime_t ktime = ktime_set(0, MS_TO_NS(500));
	...
	if (BMT_status.bat_charging_state == CHR_BATFULL)
		ret = rt9466_check_backboost(info);
	else
		hrtimer_start(&info->timer, ktime, HRTIMER_MODE_REL);
...
//CEI comment start//
	for (i = 0; i < ARRAY_SIZE(rt9466_reg_addr)/9; i++) {
		int j;
		int rt9466_reg_buf[10];
		memset(rt9466_reg_buf, 0x0, sizeof(rt9466_reg_buf));
		if(i < (ARRAY_SIZE(rt9466_reg_addr)/9) -1)
		{
			for(j = 0; j < 9; j++)
			{
				rt9466_reg_buf[j] = rt9466_i2c_read_byte(info, rt9466_reg_addr[(i*9)+j]);
			}

			dev_info(info->dev, "LE(K)=> [rt9466 reg@][0x%x]=0x%x [0x%x]=0x%x [0x%x]=0x%x [0x%x]=0x%x [0x%x]=0x%x [0x%x]=0x%x [0x%x]=0x%x [0x%x]=0x%x [0x%x]=0x%x\n",
						rt9466_reg_addr[(i*9)+0], rt9466_reg_buf[0], rt9466_reg_addr[(i*9)+1], rt9466_reg_buf[1], rt9466_reg_addr[(i*9)+2], rt9466_reg_buf[2], 
						rt9466_reg_addr[(i*9)+3], rt9466_reg_buf[3], rt9466_reg_addr[(i*9)+4], rt9466_reg_buf[4], rt9466_reg_addr[(i*9)+5], rt9466_reg_buf[5],
						rt9466_reg_addr[(i*9)+6], rt9466_reg_buf[6], rt9466_reg_addr[(i*9)+7], rt9466_reg_buf[7], rt9466_reg_addr[(i*9)+8], rt9466_reg_buf[8]);		
		}
		else if(i == ((ARRAY_SIZE(rt9466_reg_addr)/9) -1))
		{
			for(j = 0; j < 10; j++)
			{
				rt9466_reg_buf[j] = rt9466_i2c_read_byte(info, rt9466_reg_addr[(i*9)+j]);
			...
			dev_info(info->dev, "LE(K)=> [rt9466 reg@][0x%x]=0x%x [0x%x]=0x%x [0x%x]=0x%x [0x%x]=0x%x [0x%x]=0x%x [0x%x]=0x%x [0x%x]=0x%x [0x%x]=0x%x [0x%x]=0x%x [0x%x]=0x%x\n",
						rt9466_reg_addr[(i*9)+0], rt9466_reg_buf[0], rt9466_reg_addr[(i*9)+1], rt9466_reg_buf[1], rt9466_reg_addr[(i*9)+2], rt9466_reg_buf[2], 
						rt9466_reg_addr[(i*9)+3], rt9466_reg_buf[3], rt9466_reg_addr[(i*9)+4], rt9466_reg_buf[4], rt9466_reg_addr[(i*9)+5], rt9466_reg_buf[5],
						rt9466_reg_addr[(i*9)+6], rt9466_reg_buf[6], rt9466_reg_addr[(i*9)+7], rt9466_reg_buf[7], rt9466_reg_addr[(i*9)+8], rt9466_reg_buf[8], rt9466_reg_addr[(i*9)+9], rt9466_reg_buf[9]);		
...
//CEI comment start: Fix for NONSTANDARD_CHARGER or Type-C//
	[CHARGING_CMD_SET_DEFAULT_DPM] = rt_set_default_dpm,
//CEI comments end//
//CEI comments start, identify which charegr IC is using
	[CHARGING_CMD_WHO_AM_I] = rt_who_am_i,
//CEI comments end, identify which charegr IC is using
//CEI comment start, SW workaround for  IEOC accuracy
	[CHARGING_CMD_SET_IEOC] = rt_set_ieoc,
//CEI comment end, SW workaround for  IEOC accuracy
...
//CEI comment start
//Add 500ms thread to prevent backboost
	[CHARGING_CMD_PLUG_IN] = rt_charger_plug_in,
	[CHARGING_CMD_PLUG_OUT] = rt_charger_plug_out,
	[CHARGING_CMD_CHECK_BACKBOOST] = rt_charger_check_backboost,
//CEI comment end
...
//CEI comments start, disable system reset function
	ret = rt9466_set_bit(info, RT9466_REG_CHG_CTRL19, RT9466_MASK_PROFF_RST_DIS);
//CEI comments end, disable system reset function
...
//Add 500ms thread to prevent backboost
	...
	/* create thread */
	info->thread = kthread_create(rt9466_thread_handler, info,
		"rt9466_thread");
	...
	info->timer.function = rt9466_timer_callback;
	hrtimer_start(&info->timer, ktime, HRTIMER_MODE_REL);
//CEI comment end
