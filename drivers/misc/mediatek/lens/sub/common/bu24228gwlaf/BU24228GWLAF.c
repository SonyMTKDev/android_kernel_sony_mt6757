...
#define AF_DEBUG
#ifdef AF_DEBUG
#define LOG_DBG(format, args...) pr_debug(AF_DRVNAME " [%s] " format, __func__, ##args)
...
static int OIS_DownloadData(int data_no)
{
	int i;
...
static inline int moveAF(unsigned long a_u4Position)
{
	...
	if (*g_pAF_Opened == 1) 
	{
		...
		if(initState == true)
		{
			LOG_DBG("Skip OIS Init\n");
		}
		else //Init OIS
		{
			LOG_DBG("Init OIS Only One time~\n");
	...
	LOG_DBG("move [curr] %ld [target] %ld\n", g_u4CurrPosition, g_u4TargetPosition);
	...
	if (AF_Position_WriteI2C(0x60F2, g_u4TargetPosition) == 0) {
		spin_lock(g_pAF_SpinLock);
		g_u4CurrPosition = g_u4TargetPosition;
		spin_unlock(g_pAF_SpinLock);
	}
...
static inline int setOIS(struct stAF_MotorCmd *pstMotorCmd)
{
	u8 servoState = 0;
	u8 oisMode = 0;
	...
	if(initState == true) {
		...
		LOG_INF("Waiting for OIS init done...");
...
long BU24228GWLAF_Ioctl(struct file *a_pstFile, unsigned int a_u4Command, unsigned long a_u4Param)
{
	...
	case AFIOC_S_SETPARA:
		i4RetValue = setOIS((struct stAF_MotorCmd *) (a_u4Param));
...
int BU24228GWLAF_SetI2Cclient(struct i2c_client *pstAF_I2Cclient, spinlock_t *pAF_SpinLock, int *pAF_Opened)
{
