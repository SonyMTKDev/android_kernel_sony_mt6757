...
#define PROC_CAMERA_ID "driver/camsensorid"
		...
		for (i = KDIMGSENSOR_INVOKE_DRIVER_0; i < KDIMGSENSOR_MAX_INVOKE_DRIVERS; i++) {
			if (g_invokeSocketIdx[i] != DUAL_CAMERA_NONE_SENSOR) {
				...
				if (sensorID == 0) {	/* not implement this feature ID */
					...
					else if(sensorID == IMX219_CHICONY_SENSOR_ID)
					{
						snprintf(mtk_cid_name, sizeof(mtk_cid_name), "%s, %x",mtk_cid_name, 0x77);
					}
					else if(sensorID == IMX234_SENSOR_ID)
					{
						snprintf(mtk_cid_name, sizeof(mtk_cid_name), "%s, %x",mtk_cid_name, 0x73);
					}
					else
					{
						PK_INF("Searching sensor ID=0x%x\n", sensorID);
					}
...
static int __init CAMERA_HW_i2C_init(void)
{
    ...
    proc_create(PROC_CAMERA_ID, 0, NULL, &fcamera_proc_fopsReturnId);
