...
#if 1
			CCCI_ERROR_LOG(md_id, RPC,"[CEI] source pin = %d\n", value);
			cei_sim_id = get_cei_sim_id();
			CCCI_ERROR_LOG(md_id, RPC, "get_eint_attr_val start! sim id = %d\n", cei_sim_id);

			if( cei_sim_id == CEI_SIM_SS){
				value = 1;
				CCCI_ERROR_LOG(md_id, RPC,"[CEI] SS image, set eint source pin = %d\n", value);
			}else if( cei_sim_id == CEI_SIM_DS ){
				// DS
				value = 2;
				CCCI_ERROR_LOG(md_id, RPC,"[CEI] DS image, set eint source pin = %d\n", value);
			}else{
				CCCI_ERROR_LOG(md_id, RPC,"[CEI] Unknown variants\n");
			}
#endif
