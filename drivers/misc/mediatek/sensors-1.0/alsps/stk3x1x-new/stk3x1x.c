...
#ifdef STK_CHECK
	struct workqueue_struct *stk_ps_check; /* 20170307 add */
	struct delayed_work stk_ps_check_work; /* 20170307 add */
#endif
...
int stk3x1x_init_all_reg(struct i2c_client *client)
{
	...
	APS_ERR("stk3x1x_init_all_reg start \n");
...
static int stk3x1x_validate_n_handle(struct i2c_client *client)
{
	...
	APS_ERR("%s: LT= %2X, \n", __FUNCTION__, atomic_read(&obj->ps_low_thd_val));
        return 0;
}
...
// CCI add First boot H/L thd for debug start
	if(enable){
#ifdef STK_CHECK
		queue_delayed_work(obj->stk_ps_check, &obj->stk_ps_check_work, 0); /*20161212 add */
#endif
...
#ifdef STK_CHECK
			cancel_delayed_work_sync(&obj->stk_ps_check_work);
#endif
		}
// CCI add First boot H/L thd for debug end
...
static int stk_ps_tune_zero_func_fae(struct stk3x1x_priv *obj)
{
	...
	if(diff > STK_MAX_MIN_DIFF && diff > temp_max_min_diff)
	{
		...
		else
		{
			...
			printk(KERN_INFO "%s:psi_set(%d) and cci_ps_high_thd(%d)\n", __func__, obj->psi_set, cci_ps_high_thd);
...
#ifdef STK_CHECK
/* 20161212 add */
static void stk_ps_check_work_func(struct work_struct *work)
{
	struct stk3x1x_priv *obj = stk3x1x_obj;
	int err_wait=0, err_init=0, err=0, ret=0;
	u8 reg_wait=0x00;
	u8 rbuf[6]={0,0,0,0,0,0};
	u8 tbuf[4]={0,0,0,0};
	u8 abuf[4]={0,0,0,0};
	...
	APS_ERR("[Colby]stk3x1x_p_check_handler ps = %d\n", obj->ps);

	ret = stk3x1x_master_recv(obj->client, 0, &rbuf[0], 6);
	ret = stk3x1x_master_recv(obj->client, 6, &tbuf[0], 4);
	if(ret < 0){
		APS_ERR("[Colby]stk3x1x_p_check error \n");
	}
	APS_LOG("%s: stk3x1x_p_check state = %2X, psc = %2X, alsc=%2X, ledc=%2X, INT=%2X, WAIT=%2X\n", __FUNCTION__, rbuf[0], rbuf[1], rbuf[2], rbuf[3], rbuf[4], rbuf[5]);
	APS_LOG("%s: stk3x1x_p_check HT = %2X,%2X LT = %2X,%2X \n", __FUNCTION__, tbuf[0], tbuf[1], tbuf[2] ,tbuf[3]);
	err_wait = stk3x1x_read_wait(obj->client, &reg_wait);
	APS_ERR("[Colby]stk3x1x_p_check_handler reg_wait = %d\n", reg_wait);

	if(err_wait < 0) {
		APS_ERR("stk3x1x_p_check_handler get wait value: %d\n", err_wait);
	} else if (reg_wait != obj->wait_val) {
		APS_ERR("stk3x1x p_check_handler wait init \n");
		if((err_init = stk3x1x_validate_n_handle(obj->client))) {
			APS_ERR("stk3x1x init (PS_ENABLE for ESD reset) failed: %d\n", err_init);
		}
	}

	if (tbuf[0] == 0xff) {
		APS_ERR("stk3x1x p_check_handler HT init \n");
		if((err_init = stk3x1x_write_ps_high_thd(obj->client, atomic_read(&obj->ps_high_thd_val)))) {
			APS_ERR("write high thd error: %d\n", err_init);
		}
		if((err_init = stk3x1x_write_ps_low_thd(obj->client, atomic_read(&obj->ps_low_thd_val)))) {
			APS_ERR("write low thd error: %d\n", err_init);
		}
		ret = stk3x1x_master_recv(obj->client, 6, &abuf[0], 4);
		APS_LOG("%s: After stk3x1x_p_check HT = %2X,%2X LT = %2X,%2X \n", __FUNCTION__, abuf[0], abuf[1], abuf[2] , abuf[3]);

		if((err = ps_report_interrupt_data(1))) {
			APS_ERR("ps call hwmsen_get_interrupt_data fail = %d\n", err);
		}
	}

	queue_delayed_work(obj->stk_ps_check, &obj->stk_ps_check_work, 1000); /*20161212 add */
	return;
}
/* 20161212 add */
...
void stk_ps_key_check_func(void)
{
	struct stk3x1x_priv *obj = stk3x1x_obj;

	APS_LOG("stk3x1x ps enable state: %d\n", STK_EN);

	if (STK_EN == 1) {
		wake_lock_timeout(&ps_lock, 5 * HZ);
		cancel_delayed_work_sync(&obj->stk_ps_check_work);
		APS_LOG("stk3x1x key check start \n");
		queue_delayed_work(obj->stk_ps_check, &obj->stk_ps_check_work, 0);
	}
}
EXPORT_SYMBOL(stk_ps_key_check_func);
...
{	
	...
	if (IS_ERR(ps_data->pinctrl)) {
		ret = PTR_ERR(ps_data->pinctrl);
		printk("Cannot find alsps pinctrl!\n");
		return ret;
	}
...
static ssize_t stk3x1x_store_cali(struct device_driver *ddri, const char *buf,size_t size)
{
	...
	unsigned int tmp_low_thd=0,tmp_high_thd=0;
	...
	else if(hwid == BY61_hwid)
	{
		tmp_high_thd = value;
		cci_ps_high_thd = value + 150;
		if ((cci_ps_high_thd > cci_ps_cover) && (cci_ps_cover != 0))
			cci_ps_high_thd = cci_ps_cover;

		printk("%s:[STK]loren test tmp_high_thd=%d cci_ps_high_thd=%d cci_ps_cover=%d\n",__func__,tmp_high_thd,cci_ps_high_thd,cci_ps_cover);
	}
	else
	...
	else if(hwid == BY61_hwid)
	{
		...
		if(tmp_low_thd > tmp_high_thd)
			printk("%s:[STK]ERROR calibration data,high_thd=%d ,low_thd=%d\n",__func__,tmp_high_thd,tmp_low_thd);

		cci_ps_low_thd = cci_ps_high_thd - (tmp_high_thd - tmp_low_thd);
		printk("%s:[STK]loren test tmp_low_thd=%d cci_ps_low_thd=%d \n",__func__,tmp_low_thd,cci_ps_low_thd);
	}
	else
...
static ssize_t stk3x1x_show_ServiceCenter(struct device_driver *ddri, char *buf)
{
	...
	/* Setup High/Low threshold using Golden Value and criterial */
	if (hwid == BY57_hwid) {
		/* SM11 golden value */
		cci_ps_high_thd = cci_result_ct + STK_HT_N_CT;
		cci_ps_low_thd = cci_result_ct + STK_LT_N_CT;

		/* SM11 criterial (for Service Center) */
		limit_min_ct = 0;
		limit_max_ct = 1000;
		...
		/* SM11L criterial (for Service Center) */
		limit_min_ct = 20;
		limit_max_ct = 1500;

	} else if (hwid == BY86_hwid) {
		/* SM21 golden value */
		cci_ps_high_thd = cci_result_ct + 100;
		cci_ps_low_thd = cci_result_ct + 50;

		/* SM21 criterial (for Service Center) */
		limit_min_ct = 0;
		limit_max_ct = 1000;
	...
	if ((cci_result_ct > limit_min_ct) && (cci_result_ct < limit_max_ct)) {
		atomic_set(&stk3x1x_obj->ps_high_thd_val, cci_ps_high_thd);
		atomic_set(&stk3x1x_obj->ps_low_thd_val, cci_ps_low_thd);

		if ((err = stk3x1x_write_ps_high_thd(stk3x1x_obj->client,
				atomic_read(&stk3x1x_obj->ps_high_thd_val)))) {
			APS_ERR("[STK]write high thd error: %d\n", err);
			res = -3;
		}

		if ((err = stk3x1x_write_ps_low_thd(stk3x1x_obj->client,
	...
	printk (KERN_ERR "[STK]%s : Psensor ServiceCenter calibrate CT done.  Res = %d, "
			"cci_result_ct = %d (limit: %d - %d), "
...
static ssize_t stk3x1x_show_GC_CT(struct device_driver *ddri, char *buf)
{
	...
	if( hwid ==  BY57_hwid ){
		...
		if( (cci_result_ct > 20) && (cci_result_ct < 1500) && (Diff_ThdH_L > 15) && (Diff_ThdL_CT > 15) ){
			atomic_set(&stk3x1x_obj->ps_high_thd_val, cci_ps_high_thd);
			atomic_set(&stk3x1x_obj->ps_low_thd_val, cci_ps_low_thd);
			if((err = stk3x1x_write_ps_high_thd(stk3x1x_obj->client, atomic_read(&stk3x1x_obj->ps_high_thd_val))))
			{
		...
		if( (cci_result_ct > 0) && (cci_result_ct < 1000) && (Diff_ThdL_CT > 20) && (Diff_ThdH_L > 20) ){ /* 20170214 */
			atomic_set(&stk3x1x_obj->ps_high_thd_val, cci_ps_high_thd);
			atomic_set(&stk3x1x_obj->ps_low_thd_val, cci_ps_low_thd);
		if((err = stk3x1x_write_ps_high_thd(stk3x1x_obj->client, atomic_read(&stk3x1x_obj->ps_high_thd_val))))
		{
...
static ssize_t stk3x1_cali_Light(struct device_driver *ddri, char *buf)
{
	...
	if( hwid ==  BY57_hwid )
		...
		result = (ret && (cci_als_adc_cali >= 130) && ( cci_als_adc_cali <= 673))? 1:0;
	}
	else if(hwid ==  BY86_hwid){
...
static int stk3x1x_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (err < 0) {
		...
		goto exit_from_get_alsps_dts_func; //change "exit:" to "exit_from_get_alsps_dts_func"
	}
...
//	client->timing=400;
	...
	/* 20161212 add */
#ifdef STK_CHECK
	obj->stk_ps_check = create_singlethread_workqueue("stk_ps_check");
	INIT_DELAYED_WORK(&obj->stk_ps_check_work, stk_ps_check_work_func);
#endif
	/* 20161212 add */
...
#endif
#ifdef STK_CHECK
	wake_lock_init(&ps_lock,WAKE_LOCK_SUSPEND,"ps wakelock");//ps wakelock init
#endif
...
#ifdef STK_CHECK
	cancel_delayed_work_sync(&obj->stk_ps_check_work);
	destroy_workqueue(obj->stk_ps_check);
#endif
#endif		
