...
//CEI comment start
//record reason and charging time when CHR_ERROR happen
unsigned int CHR_ERROR_reason[5] = {0};
unsigned int CHR_ERROR_charging_time[5] = {0};
//CEI comment end
...
kal_bool bat_resume_flag = KAL_FALSE; //CEI comment, fix screen on too late due to PE detection

#ifdef CONFIG_CHARGER_QNS
#if defined(CONFIG_BUILD_REDWOOD) // redwood
#define BATTERY_TYPE_NAME_SEND "1305-9573"
#define BATTERY_TYPE_NAME_ATL "Dummy"
#define BATTERY_TYPE_NAME_DEFAULT "Dummy"
#elif defined(CONFIG_BUILD_TEAK) // teak
#define BATTERY_TYPE_NAME_SEND "1308-3580"
#define BATTERY_TYPE_NAME_ATL "Dummy"
#define BATTERY_TYPE_NAME_DEFAULT "Dummy"
#else // hinoki
/* Battery type name */
#define BATTERY_TYPE_NAME_SEND "1304-7760"
#define BATTERY_TYPE_NAME_ATL "1304-7769"
#define BATTERY_TYPE_NAME_DEFAULT "Dummy"
#endif
#endif
...
int enable_llk;
/* CEI comment end, FP22589 Prevent swelling */
//CEI comment start, BATTERY_CARE//
#if defined(CONFIG_BUILD_TEAK) // teak
	int smart_enabled;
	int smart_suspended;
#endif
//CEI comment end, BATTERY_CARE//
	/* Dual battery */
	int status_smb;
	int capacity_smb;
	int present_smb;
...
// too many battery_props will cause -ENOMEM from add_uevent_var(), mark unnesessary props for teak
#else
...
static DEFINE_MUTEX(smart_charge_lock);
int get_smart_ibat_chg_en(void);
void set_smart_ibat_chg_en(int smart_ibat_chg_en);
static void smart_charge_wdog_work(struct work_struct *work);
static int chg_smart_set_suspend(int smart_suspended);

struct delayed_work wdog_work;
static int g_smart_ibat_chg_en = 1;
#endif
//CEI comment end, BATTERY_CARE//
				...
				enum power_supply_property psp, const union power_supply_propval *val)
{
	...
	case POWER_SUPPLY_PROP_ENABLE_LLK:
		...
		battery_log(BAT_LOG_CRTI, "CEI(K)=> battery_set_property: enable_llk=%d\n", data->enable_llk);
		break;
//CEI comment start, BATTERY_CARE//
#if defined(CONFIG_BUILD_TEAK) // teak
	case POWER_SUPPLY_PROP_SMART_CHARGING_ACTIVATION:
		if (val->intval) {
			...
			battery_log(BAT_LOG_CRTI, "CEI(K)=> [SMRT] Smart Charging was activated, smart_enabled=%d\n", data->smart_enabled);
		}
		break;
	case POWER_SUPPLY_PROP_SMART_CHARGING_INTERRUPTION:
		if (data->smart_enabled) {
			data->smart_suspended = (bool)val->intval;
			chg_smart_set_suspend(data->smart_suspended);

			battery_log(BAT_LOG_CRTI, "CEI(K)=> [SMRT] smart_enabled=%d, set smart_suspended=%d ok.\n",
				data->smart_enabled, data->smart_suspended);
		}
		else
		{
			battery_log(BAT_LOG_CRTI, "CEI(K)=> [SMRT] Illegal operation!! smart_enabled=%d, but attempt to set smart_suspended=%d\n",
				data->smart_enabled, val->intval);
		}
		break;
#endif
//CEI comment end, BATTERY_CARE//
...
static void smart_charge_wdog_work(struct work_struct *work)
{
	...
	battery_log(BAT_LOG_CRTI,  "CEI(K)=> [SMRT] Smart Charge Watchdog timer has expired.\n");

	mutex_lock(&smart_charge_lock);
	set_smart_ibat_chg_en(1);
	bat_data->smart_suspended = false;
	mutex_unlock(&smart_charge_lock);
}
...
static int chg_smart_set_suspend(int smart_suspended)
{
	...
	if (!bat_data->smart_enabled) {
		battery_log(BAT_LOG_CRTI, "CEI(K)=> [SMRT] Couldn't set smart charge voter due to unactivated\n");
		goto exit;
	}

	if(smart_suspended == 1)
		set_smart_ibat_chg_en(0);
	else
		set_smart_ibat_chg_en(1);

	//Prevent any mistake, ensure ibat is enabled before cancelling the wdog work
	if( ((smart_suspended == 0) && (g_smart_ibat_chg_en == 1) ) || (smart_suspended == 1) )
		cancel_delayed_work_sync(&wdog_work);

	if (smart_suspended) {
		schedule_delayed_work(&wdog_work,
...
int get_smart_ibat_chg_en(void)
{
	...
	battery_log(BAT_LOG_CRTI, "CEI(K)=> [SMRT] get_smart_ibat_chg_en(), g_smart_ibat_chg_en=%d, smart_enabled=%d, smart_suspended=%d",
		g_smart_ibat_chg_en, bat_data->smart_enabled, bat_data->smart_suspended);

	return g_smart_ibat_chg_en;
}

void set_smart_ibat_chg_en(int smart_ibat_chg_en)
{
	g_smart_ibat_chg_en = smart_ibat_chg_en;
	if(g_smart_ibat_chg_en == 0)
	{
		battery_log(BAT_LOG_CRTI, "CEI(K)=> [SMRT] @@@ DISABLE IBAT!@@@ g_smart_ibat_chg_en=%d\n", g_smart_ibat_chg_en);
	}
	else if (g_smart_ibat_chg_en == 1)
	{
		battery_log(BAT_LOG_CRTI, "CEI(K)=> [SMRT] @@@ ENABLE IBAT!@@@ g_smart_ibat_chg_en=%d\n", g_smart_ibat_chg_en);
	}
}
#endif
//CEI comment end, BATTERY_CARE//
...
{
	battery_log(BAT_LOG_CRTI, "show_SC30_Status: en=%d TA=%u, TB=%u, TC=%u, TT=%u, low_cv=%d\n",
		sc30_en, time_A, time_B, time_C, time_T, low_cv);

	return sprintf(buf, "SC30 en=%d TA=%u, TB=%u, TC=%u, TT=%u, low_cv=%d\n", 
		sc30_en, time_A, time_B, time_C, time_T, low_cv);
}
				  ...
				  const char *buf, size_t size)
{
	if (sscanf(buf, "%u %u %u %u %u", &time_A, &time_B, &time_C, &time_T, &low_cv) == 5) {
		...
		battery_log(BAT_LOG_CRTI, "store_SC30_Status: TA=%u, TB=%u, TC=%u, TT=%u, low_cv=%d\n",
...
const int g_charge_full_design_SM11 = 2300; //mAh
const int g_charge_full_design_SM21 = 2700; //mAh
const int g_charge_full_design_SM11L = 3430; //mAh
int g_age_counter = 0;
int g_age_mtkcalc = 0;
int g_age_avgmiscta = 0;
int g_charge_full = 0;
...
static ssize_t show_charge_full_design(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	if(customer_project_id == 1)
		return sprintf(buf, "%d\n", g_charge_full_design_SM21);
	else if(customer_project_id == 3)
		return sprintf(buf, "%d\n", g_charge_full_design_SM11L);
	else
		return sprintf(buf, "%d\n", g_charge_full_design_SM11);
}
static ssize_t store_charge_full_design(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t size)
{
	battery_log(BAT_LOG_CRTI, "%s, Not Support Write Function!\n", __func__);

	return size;
}
static DEVICE_ATTR(charge_full_design, 0664, show_charge_full_design, store_charge_full_design);
//---
static ssize_t show_age_counter(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", g_age_counter);
}
static ssize_t store_age_counter(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t size)
{
	battery_log(BAT_LOG_CRTI, "%s, Not Support Write Function!\n", __func__);
...
static DEVICE_ATTR(age_counter, 0664, show_age_counter, store_age_counter);
//---
static ssize_t show_age_mtkcalc(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", g_age_mtkcalc);
}
static ssize_t store_age_mtkcalc(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t size)
{
	battery_log(BAT_LOG_CRTI, "%s, Not Support Write Function!\n", __func__);
...
static DEVICE_ATTR(age_mtkcalc, 0664, show_age_mtkcalc, store_age_mtkcalc);
//---
static ssize_t show_age_avgmiscta(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", g_age_avgmiscta);
}
static ssize_t store_age_avgmiscta(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t size)
{
	if (kstrtouint(buf, 10, &g_age_avgmiscta) == 0) {
		g_charge_full = g_age_avgmiscta;
		battery_log(BAT_LOG_CRTI, "%s: set g_age_avgmiscta=%d, g_charge_full=%d\n", __func__, g_age_avgmiscta, g_charge_full);
	} else
		battery_log(BAT_LOG_CRTI, "%s, argument ERROR\n", __func__);

	return size;
}
static DEVICE_ATTR(age_avgmiscta, 0664, show_age_avgmiscta, store_age_avgmiscta);
//---
static ssize_t show_age_testadd(struct device *dev, struct device_attribute *attr, char *buf)
{
	u32 random_value = 0;
	int customer_project_id = get_cei_customer_project_id();
	...
	if(customer_project_id == 1)
		g_age_mtkcalc = (random_value % (g_charge_full_design_SM21 - 99)) + 100; // 100 ~ g_charge_full_design
	else if(customer_project_id == 3)
		g_age_mtkcalc = (random_value % (g_charge_full_design_SM11L - 99)) + 100; // 100 ~ g_charge_full_design
	else
		g_age_mtkcalc = (random_value % (g_charge_full_design_SM11 - 99)) + 100; // 100 ~ g_charge_full_design
	g_age_counter = g_age_counter + 1;

	return sprintf(buf, "Success! mtkcalc=%d, counter=%d\n", g_age_mtkcalc, g_age_counter);
}
static ssize_t store_age_testadd(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t size)
{
	battery_log(BAT_LOG_CRTI, "%s, Not Support Write Function!\n", __func__);
...
static DEVICE_ATTR(age_testadd, 0664, show_age_testadd, store_age_testadd);
//---
static ssize_t show_charge_full(struct device *dev, struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", g_charge_full);
}
static ssize_t store_charge_full(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t size)
{
	battery_log(BAT_LOG_CRTI, "%s, Not Support Write Function!\n", __func__);
...
static DEVICE_ATTR(charge_full, 0664, show_charge_full, store_charge_full);
//CEI comment end//

//CEI comments start, identify which charegr IC is using
static ssize_t show_ChargerIC_Whoami(struct device *dev, struct device_attribute *attr, char *buf)
{
	unsigned int chargeric_whoami = 0;
...
static ssize_t store_ChargerIC_Whoami(struct device *dev, struct device_attribute *attr,
				  const char *buf, size_t size)
{	
	return 0;
}

static DEVICE_ATTR(ChargerIC_Whoami, 0664, show_ChargerIC_Whoami, store_ChargerIC_Whoami);
//CEI comments start, identify which charegr IC is using
...
//CEI comments S
	battery_log(BAT_LOG_CRTI, "CEI(K)=> mt_battery_GetBatteryData (V_BAT_SENSE)=%d, (V_I_SENSE)=%d, meta=%d, nv=%d\n", bat_vol, Vsense, meta_car_tune, nvram_car_tune);
//CEI comments E
}
...
//record reason and charging time when CHR_ERROR happen
			for(i = 0; i < 4; i++)
			{
				CHR_ERROR_reason[i] = CHR_ERROR_reason[i+1];
				CHR_ERROR_charging_time[i] = CHR_ERROR_charging_time[i+1];
			}
			CHR_ERROR_reason[4] = 1;
			CHR_ERROR_charging_time[4] = BMT_status.total_charging_time;
			battery_log(BAT_LOG_CRTI,"JA(K)=> set CHR_ERROR_reason=%d %d %d %d %d, CHR_ERROR_charging_time=%d %d %d %d %d\n", CHR_ERROR_reason[0], CHR_ERROR_reason[1], CHR_ERROR_reason[2], CHR_ERROR_reason[3], CHR_ERROR_reason[4], CHR_ERROR_charging_time[0], CHR_ERROR_charging_time[1], CHR_ERROR_charging_time[2], CHR_ERROR_charging_time[3], CHR_ERROR_charging_time[4]);
//CEI comment end
		}
#endif
#if !defined(CONFIG_MTK_DUAL_INPUT_CHARGER_SUPPORT)
...
#else
		if(BMT_status.charger_type == STANDARD_HOST){
			//Safty timer for USB = 20hr
			if(BMT_status.total_charging_time >= MAX_CHARGING_TIME){
				battery_log(BAT_LOG_CRTI, "[BATTERY]CEI(K)=> Charging Over Time! charger_type %d, total_charging_time %d\n",BMT_status.charger_type,BMT_status.total_charging_time);
				status = PMU_STATUS_FAIL;
			}
		}else{
			//Safty timer for the other charger types = 8hr
			if(BMT_status.total_charging_time >= MAX_AC_CHARGING_TIME){
				battery_log(BAT_LOG_CRTI, "[BATTERY]CEI(K)=> Charging Over Time! charger_type %d, total_charging_time %d\n",BMT_status.charger_type,BMT_status.total_charging_time);
				status = PMU_STATUS_FAIL;
			}
 		}
...
//CEI comment start
//safety timer
	battery_log(BAT_LOG_FULL, "[BATTERY]CEI(K)=> mt_battery_CheckChargingTime, MAX_CHARGING_TIME %d, MAX_AC_CHARGING_TIME %d\n", MAX_CHARGING_TIME, MAX_AC_CHARGING_TIME);

	battery_log(BAT_LOG_CRTI, "[BATTERY]CEI(K)=> mt_battery_CheckChargingTime, chr_type %d, tot_chg_time %d, g_battery_thermal_throttling_flag %d, status %d\n"
		,BMT_status.charger_type ,BMT_status.total_charging_time,g_battery_thermal_throttling_flag, status);
//CEI comment end
...
//Safety_timer
		//if (BMT_status.total_charging_time >= MAX_CHARGING_TIME)
		if( (((BMT_status.charger_type == STANDARD_CHARGER) || (BMT_status.charger_type == APPLE_2_1A_CHARGER) ) && BMT_status.total_charging_time >= MAX_AC_CHARGING_TIME) ||\
			(((BMT_status.charger_type != STANDARD_CHARGER) && (BMT_status.charger_type != APPLE_2_1A_CHARGER)) && (BMT_status.total_charging_time >= MAX_CHARGING_TIME)) )
//CEI comments end//
...
void do_chrdet_int_task(void)
{
	...
	if (g_bat_init_flag == KAL_TRUE) {
		...
		if ((DISO_data.diso_state.cur_vusb_state == DISO_ONLINE) ||
			...
			BMT_status.charger_exist = KAL_TRUE;
//CEI comment start
//Add 500ms thread to prevent backboost
			battery_charging_control(CHARGING_CMD_PLUG_IN, NULL);
//CEI comment end
...
//CEI comment start
//Add 500ms thread to prevent backboost
			battery_charging_control(CHARGING_CMD_PLUG_OUT, NULL);
//CEI comment end
...
//CEI comments, soft charge 3.0 start
	...
	if ((prev_charger_status == KAL_TRUE) &&
		...
		//to avoid overflow S
		if ( time_A >= 2147482648) {
			time_A = 2147482648;
			battery_log(BAT_LOG_CRTI,"[SC30] limit time_A %u\n ", time_A);
		} else if (time_B >= 2147482648) { 
			time_B = 2147482648;
			battery_log(BAT_LOG_CRTI,"[SC30] limit time_B %u\n ", time_B);
		} else if (time_C >= 2147482648) {
			time_C = 2147482648;
			battery_log(BAT_LOG_CRTI,"[SC30] limit time_C %u\n ", time_C);
		} else if (time_T >= 2147482648) {
			time_T = 2147482648;
			battery_log(BAT_LOG_CRTI,"[SC30] limit time_T %u\n ", time_T);
		}
		//to avoid overflow E
		
		if ((time_T > SC30_TIME_THRESHOLD) && (BMT_status.UI_SOC2 == 100 || BMT_status.UI_SOC2 == 1))
			low_cv = 1;
		...
		battery_log(BAT_LOG_CRTI, "[SC30] d=%u C=%d V=%d T=%d UISOC2=%d (TA=%u TB=%u TC=%u TT=%u LCV=%u)\n",
			thread_duration_time, BMT_status.charger_exist, BMT_status.bat_vol, temperature,
			BMT_status.UI_SOC2, time_A, time_B, time_C, time_T, low_cv);
	} else if (sc30_en == KAL_TRUE) {
		if ((time_T > SC30_TIME_THRESHOLD) && (BMT_status.UI_SOC2 == 100 || BMT_status.UI_SOC2 == 1))
			low_cv = 1;
		...
		battery_log(BAT_LOG_CRTI, "[SC30] UISOC2=%d (TA=%u TB=%u TC=%u TT=%u LCV=%u)\n",
			BMT_status.UI_SOC2, time_A, time_B, time_C, time_T, low_cv);
	} else
		set_sc30_low_cv_voltage(KAL_FALSE);
...
//This ioctl cmd changes present car_tune_value, need to leave a log
		meta_car_tune = adc_in_data[1] / 1000;
		battery_log(BAT_LOG_CRTI, "LE(KE)=> Set_BAT_CAR_TUNE_VALUE[%d], res=%d !!! (WARNING)\n",
...
//parameter setting
	...
	//#endif
	if(customer_project_id == 1)
		batt_cust_data.ta_ac_charging_current = TA_AC_CHARGING_CURRENT_SM21;
	else if(customer_project_id == 3)
		batt_cust_data.ta_ac_charging_current = TA_AC_CHARGING_CURRENT_SM11L;
	else
		batt_cust_data.ta_ac_charging_current = TA_AC_CHARGING_CURRENT_SM11;
//CEI comments end
...
//CEI comment start, BATTERY_CARE//
#if defined(CONFIG_BUILD_TEAK) // teak
	INIT_DELAYED_WORK(&wdog_work,
					smart_charge_wdog_work);
#endif
//CEI comment end, BATTERY_CARE//
...
//CEI comments, soft charge 3.0 end
//CEI comments start, identify which charegr IC is using
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_ChargerIC_Whoami);
//CEI comments end, identify which charegr IC is using
//CEI comment start//
//battery health(aging factor)
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_charge_full_design);
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_age_counter);
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_age_mtkcalc);
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_age_avgmiscta);
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_age_testadd);
		ret_device_file = device_create_file(&(dev->dev), &dev_attr_charge_full);
//CEI comment end//
	}
...
static void battery_timer_resume(void)
{
	...
	if (is_pcm_timer_trigger == KAL_TRUE || bat_spm_timeout || battery_meter_get_low_battery_interrupt_status()) {
		mutex_lock(&bat_mutex);
		battery_meter_reset_sleep_time();
		bat_resume_flag=KAL_TRUE;//CEI comment, fix screen on too late due to PE detection
		BAT_thread();
		mutex_unlock(&bat_mutex);
	} else {
	...
	bat_resume_flag=KAL_FALSE;//CEI comment, fix screen on too late due to PE detection
	mutex_unlock(&bat_mutex);
