...
//CEI comment start
//revise coding style of fault status in BQ25896
...
//CEI comment start
//revise coding style of fault status in BQ25896
//static const char *bq25896_chg_fault_status_name[] = {
...
static const rt_register_map_t bq25896_regmap_map[] = {
	RT_REG(BQ25896_CON0),
	RT_REG(BQ25896_CON1),
	RT_REG(BQ25896_CON2),
	RT_REG(BQ25896_CON3),
...
//CEI comment start
//add pe_access_lock for bq25896_check_backboost
...
//CEI comments start, workaround for BQ25896 power drop
unsigned int bq25896_pre_inlim = 150000;/* 10uA */
unsigned int bq25896_workaround =0;
unsigned int bq25896_screen_on =0;
//CEI comments end, workaround for BQ25896 power drop
...
//CEI comment start//
//Disable bq25896 IC JEITA for SP2 build for SW workaround
...
//CEI comment start
//revise coding style of fault status in BQ25896
	//bq_charger_dump_register(mchr_info, NULL);
//CEI comment end
	return ret;
}
...
//battery/charging bring up
	...
	battery_log(BAT_LOG_CRTI, "%s: enable = %d\n", __func__, enable);
	ret = (enable ? bq25896_set_bit : bq25896_clr_bit)
		(info, BQ25896_CON0, CON0_EN_HIZ_MASK);
#endif
//CEI comment end
...
static int bq_set_default_dpm(struct mtk_charger_info *mchr_info, void *data)
{
	struct bq25896_info *info = (struct bq25896_info *)mchr_info;
	int ret = 0;
	u32 vindpm = *((u32 *)data);

	bq25896_set_force_vindpm(info, true);
	if (ret == 0)
		ret = bq25896_set_vindpm(info, vindpm);
...
//CEI comments start, workaround for BQ25896 power drop
static int bq_charger_set_workaround_temp_iinlim(struct mtk_charger_info *mchr_info,
	void *data)
{
	int ret = 0;
	u32 iinlim = *((u32 *)data);
	
	bq25896_pre_inlim = iinlim;
		
	return ret;
}

static irqreturn_t bq_irq_handler(int irq, void *data)
{
	int ret = 0, inlim_ret = 0;
	unsigned int status = 0;
	u32 iinlim = 40000;/* 10uA */
	 	
	struct mtk_charger_info *info = (struct mtk_charger_info *)data;

	battery_log(BAT_LOG_CRTI, "%s: starts\n", __func__);

	ret = bq_charger_is_charging_done(info, &status);

	if (status) {
		inlim_ret = bq_charger_set_iinlim(info, &iinlim);
		bq25896_workaround = 1;
		battery_log(BAT_LOG_CRTI, "[bq_workaround] %s() status = %d, bq25896_workaround = %d\n", 
			__func__, status, bq25896_workaround);
	} else {
		battery_log(BAT_LOG_CRTI, "%s() status = %d\n", __func__, status);
	}
...
static int bq_irq_register(struct bq25896_info *info)
{
	int ret = 0;
	...
	ret = request_threaded_irq(info->irq, NULL, bq_irq_handler,
		IRQF_TRIGGER_FALLING | IRQF_ONESHOT, "bq25896_irq", info);
	if (ret < 0) {
...
static struct notifier_block bq_fb_notifier;
static struct mtk_charger_info *bq_temp_info= NULL;
static int bq_fb_notifier_callback(struct notifier_block *self, unsigned long event, void *data)
{
	struct fb_event *evdata = NULL;
	int blank;
	int ret = 0;
	u32 bq_workaround_iinlim = 40000; ;/* 10uA */
	...
	if (event == FB_EVENT_BLANK) {
		...
		switch (blank) {//lcd on
			case FB_BLANK_UNBLANK:
				bq25896_screen_on = 1;
				battery_log(BAT_LOG_CRTI, "%s  lcd on \n", __func__);
				if (bq25896_workaround) {
					ret = bq_charger_set_iinlim(bq_temp_info, &bq25896_pre_inlim);
					battery_log(BAT_LOG_CRTI, "[bq_workaround]  lcd on, set %dma\n", bq25896_pre_inlim);
				}
			break;
			default:
			break;
	...
	if (event == FB_EARLY_EVENT_BLANK) {
		blank = *(int *)evdata->data;
		switch (blank) {//lcd off
			case FB_BLANK_POWERDOWN:
				bq25896_screen_on = 0;
				battery_log(BAT_LOG_CRTI, "%s  lcd off \n", __func__);
				if (bq25896_workaround) {
					ret = bq_charger_set_iinlim(bq_temp_info, &bq_workaround_iinlim);
					battery_log(BAT_LOG_CRTI, "[bq_workaround]  lcd off, set %dma\n", bq_workaround_iinlim);
				}
			break;
			default:
			break;
...
static int bq_charger_plug_out(struct mtk_charger_info *mchr_info, void *data)
{
	struct bq25896_info *info = (struct bq25896_info *)mchr_info;
...
static int bq_charger_plug_in(struct mtk_charger_info *mchr_info, void *data)
{
	struct bq25896_info *info = (struct bq25896_info *)mchr_info;
	ktime_t ktime = ktime_set(0, MS_TO_NS(500));

	battery_log(BAT_LOG_CRTI, "%s\n", __func__);
	hrtimer_start(&info->timer, ktime, HRTIMER_MODE_REL);
	return 0;
}

static int bq25896_check_backboost(struct bq25896_info *info);
static int bq_charger_check_backboost(struct mtk_charger_info *mchr_info, void *data)
{
	int ret = 0;
	struct bq25896_info *info = (struct bq25896_info *)mchr_info;
	ktime_t ktime = ktime_set(0, MS_TO_NS(500));
	...
	if (BMT_status.bat_charging_state == CHR_BATFULL)
		ret = bq25896_check_backboost(info);
	else
		hrtimer_start(&info->timer, ktime, HRTIMER_MODE_REL);
...
static enum hrtimer_restart bq25896_timer_callback(struct hrtimer *timer)
{
	struct bq25896_info *info = container_of(timer, struct bq25896_info,
		timer);
	ktime_t ktime = ktime_set(0, MS_TO_NS(500));

	/* wakeup thread */
	if (!wake_up_process(info->thread))
		battery_log(BAT_LOG_CRTI, "%s: wakeup thread fail\n", __func__);

	if (BMT_status.bat_charging_state == CHR_BATFULL)
		return HRTIMER_NORESTART;

	hrtimer_forward_now(&info->timer, ktime);
	return HRTIMER_RESTART;
}

/* Prevent back boost */
static int bq25896_try_solve_backboost(struct bq25896_info *info)
{
	int ret = 0;
	u8 data = 0;
	...
	if (BMT_status.charger_type != CHARGING_HOST &&
		...
		ret = bq25896_device_read(info->i2c, BQ25896_CON0, 1, &data);
		if (ret < 0) {
			battery_log(BAT_LOG_CRTI, "%s read hz fail(%d)\n",
				__func__, ret);
			goto out;
		}

		/* HZ on */
		data |= CON0_EN_HIZ_MASK;
		ret = bq25896_device_write(info->i2c, BQ25896_CON0, 1, &data);
		if (ret < 0)
			battery_log(BAT_LOG_CRTI, "%s hz on fail(%d)\n",
				__func__, ret);
		...
		ret = bq25896_device_write(info->i2c, BQ25896_CON0, 1, &data);
		if (ret < 0)
			battery_log(BAT_LOG_CRTI, "%s hz off fail(%d)\n",
				__func__, ret);
	} else {
		data = 0x95; /* mivr = 4.7V */
//CEI comment start: Fix bug//
		ret = bq25896_device_write(info->i2c, BQ25896_COND, 1, &data);
//CEI comment end: Fix bug//
		if (ret < 0)
			battery_log(BAT_LOG_CRTI, "%s mivr = 4.7V fail(%d)\n",
				__func__, ret);
		mdelay(10);
...
//CEI comment start: Fix bug//
		...
		ret = bq25896_device_write(info->i2c, BQ25896_COND, 1, &data);
		if (ret < 0)
			battery_log(BAT_LOG_CRTI, "%s mivr = 4.5V fail(%d)\n",
				__func__, ret);
//CEI comment end: Fix bug//
	}
out:
...
//CEI comment start
//disable bq25896_check_backboost when charge termination done
        enum bq25896_charging_status chg_stat = BQ25896_CHG_STATUS_NOT_CHARGING;
//CEI comment end
...
//CEI comment end
	...
	/* battery discharging && charger exist */
	//if (current_sign == KAL_FALSE && BMT_status.charger_exist == KAL_TRUE)//orig
	if (current_sign == KAL_FALSE && BMT_status.charger_exist == KAL_TRUE && chg_stat != BQ25896_CHG_STATUS_DONE)//CEI comments, disable bq25896_check_backboost when charge termination done
		ret = bq25896_try_solve_backboost(info);
//CEI comment start
//add pe_access_lock for bq25896_check_backboost
	mutex_unlock(&info->pe_access_lock);
//CEI comment end
...
static int bq25896_thread_handler(void *data)
{
	struct bq25896_info *info = data;

	while (!kthread_should_stop()) {
		bq25896_check_backboost(info);
...
static const mtk_charger_intf bq25896_mchr_intf[CHARGING_CMD_NUMBER] = {
//CEI comment end
//CEI comment start: Fix for NONSTANDARD_CHARGER or Type-C//
	[CHARGING_CMD_SET_DEFAULT_DPM] = bq_set_default_dpm,
//CEI comments end//
//CEI comments start, identify which charegr IC is using
...
//CEI comments start, workaround for BQ25896 power drop
	[CHARGING_CMD_SET_WORKAROUND_TEMP_INPUT_CURRENT] = bq_charger_set_workaround_temp_iinlim,
//CEI comments end, workaround for BQ25896 power drop
...
//CEI comment start
//Add 500ms thread to prevent backboost
	[CHARGING_CMD_PLUG_IN] = bq_charger_plug_in,
	[CHARGING_CMD_PLUG_OUT] = bq_charger_plug_out,
	[CHARGING_CMD_CHECK_BACKBOOST] = bq_charger_check_backboost,
//CEI comment end
...
//CEI comments start, workaround for BQ25896 power drop
	bq_temp_info = devm_kzalloc(&i2c->dev, sizeof(struct mtk_charger_info), GFP_KERNEL);
	bq_temp_info =  (struct mtk_charger_info*)info;
	ret = bq_irq_register(info);
	/* use fb_notifier */
	bq_fb_notifier.notifier_call = bq_fb_notifier_callback;
	if (fb_register_client(&bq_fb_notifier))
		battery_log(BAT_LOG_CRTI,"%s : register fb_notifier fail!\n",__func__);
//CEI comments end, workaround for BQ25896 power drop
...
//Add 500ms thread to prevent backboost
	/* create thread */
	info->thread = kthread_create(bq25896_thread_handler, info,
		"bq25896_thread");
	if (IS_ERR(info->thread)) {
		battery_log(BAT_LOG_CRTI, "%s: create thread fail\n", __func__);
	...
	info->timer.function = bq25896_timer_callback;
	hrtimer_start(&info->timer, ktime, HRTIMER_MODE_REL);
//CEI comment end
