...
struct pinctrl_state *eint_as_int, *eint_output0, *eint_output1, *rst_output0, *rst_output1, *lcdid_input;
...
void tpd_gpio_lcdid(int pin)
{
	TPD_DEBUG("[tpd]tpd_gpio_lcdid pin = %d\n", pin);
	if (pin == 54)
		pinctrl_select_state(pinctrl1, lcdid_input);
}
...
pr_err("Lomen 0.2\n");
	...
	lcdid_input = pinctrl_lookup_state(pinctrl1, "state_lcdid");
	if (IS_ERR(lcdid_input)) {
		ret = PTR_ERR(lcdid_input);
		dev_err(&pdev->dev, "fwq Cannot find touch pinctrl state_lcdid!\n");
		return ret;
	}
	TPD_DEBUG("[tpd%d] mt_tpd_pinctrl----------\n", pdev->id);
	return 0;
