	...
	*ichg = TA_AC_CHARGING_CURRENT;
	...
	if(customer_project_id == 1)
	{
		if(real_v_chr <= 8500)
		{
			...
			*ichg = TA_AC_CHARGING_CURRENT_SM21;
		}
	...
	else if(customer_project_id == 3)
	{
		...
		{
			*aicr = CHARGE_CURRENT_1600_00_MA;
			*ichg = TA_AC_CHARGING_CURRENT_SM11L;
		}
		else if((real_v_chr > 8500) && (real_v_chr < 9500))
		{
			*aicr = CHARGE_CURRENT_1500_00_MA;
	...
	else
	{
		...
		{
			*aicr = CHARGE_CURRENT_1600_00_MA;
			*ichg = TA_AC_CHARGING_CURRENT_SM11;
		}
		else if((real_v_chr > 8500) && (real_v_chr < 9500))
		{
			*aicr = CHARGE_CURRENT_1500_00_MA;
	...
	battery_log(BAT_LOG_CRTI, "%s() [PE+2.0] real_v_chr=%d, aicr=%d, ichg=%d\n"
		, __func__, real_v_chr, *aicr, *ichg);
#endif
//CEI comment end
