...
static long AKECS_GetConvert(int direction, struct hwmsen_convert *cvt)
{
	if (direction == 0)
	{
		return -1;
...
static int akm09918_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	data->hw = hw;
	/* akm_map is different with MTK, so do not use hwmsen_get_convert*/
	//err = hwmsen_get_convert(data->hw->direction, &data->cvt);
	
	/* for akm00916, the direction is 6 */
	data->hw->direction = 6;
