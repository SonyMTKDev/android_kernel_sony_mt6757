...
int get_car_tune_value(void)
{
	...
	static int car_tune_val = 117;
	int customer_project_id = get_cei_customer_project_id();

	if(initialed  == -1)
	{
		g_hwid_val = get_cei_hw_id(); // for pmic.c initial early use __setup to get hwid
		if(customer_project_id == 1)
		{
			switch(g_hwid_val)
			{
				case 0x0: //PDP
	...
	bm_err("LE(K)=> [get_car_tune_value] g_hwid_val=%d, car_tune_val=%d\n", g_hwid_val, car_tune_val);
	return car_tune_val;
}
//CEI comment end//
...
#define DIFFERENCE_HWOCV_RTC		60
#endif
//CEI comment end
...
//#endif
#ifndef DIFFERENCE_SWOCV_RTC_SM11_SM21
#define DIFFERENCE_SWOCV_RTC_SM11_SM21		40
#endif
#ifndef DIFFERENCE_SWOCV_RTC_SM11L
#define DIFFERENCE_SWOCV_RTC_SM11L		40
#endif
//CEI comment end
...
//CEI comment start
//battery health(aging factor)
//#define SUSPEND_CURRENT_CHECK_THRESHOLD 100	/* 10mA */
#define SUSPEND_CURRENT_CHECK_THRESHOLD 150
//CEI comment end
#endif
...
int g_battery_id_voltage_61p9k[]={1420, 1611, -1};//with 61.9k batt_id board resistor
int g_battery_id_voltage_120k[]={1210, 1430, -1};//with 120k batt_id board resistor
int g_battery_id_voltage[]={1210, 1430, -1};//initially sync 120k setting
...
void setup_battery_id_range(void)
{
	int hw_id = get_cei_hw_id();
	int customer_project_id = get_cei_customer_project_id();

if(customer_project_id == 1)
{
	if(hw_id < CEI_SM21_HW_DVT1_2) //CEI_SM21_HW_DVT1_2=SP2
	{ //61.9k board resistor
		memcpy(g_battery_id_voltage, g_battery_id_voltage_61p9k, sizeof(g_battery_id_voltage));
	}
	else // 120k board resistor
	{
		memcpy(g_battery_id_voltage,  g_battery_id_voltage_120k, sizeof(g_battery_id_voltage));
	}
}
else if(customer_project_id == 3)
{
...
{
	if(hw_id < CEI_HW_TP) //CEI_HW_TP=TP
	{ //61.9k board resistor
		memcpy(g_battery_id_voltage, g_battery_id_voltage_61p9k, sizeof(g_battery_id_voltage));
	}
	else // 120k board resistor
	...
	bm_err("CEI(K)=> hwid=%d, g_battery_id_voltage=%d   %d",
		hw_id, g_battery_id_voltage[0], g_battery_id_voltage[1]);
}
...
//CEI comments, multi bat S
	...
	for(i = 0; i < BATT_ID_MAX_RETRY; i++)
	{
		...
		if (ret != 0)
		{
			bm_print(BMLOG_INFO_LEVEL, "CEI(K)=> [fgauge_get_profile_id]id_volt read fail\n");
			id_volt_read_fail_flag = 1;
		}
		else
		{
			bm_print(BMLOG_INFO_LEVEL, "CEI(K)=> [fgauge_get_profile_id]id_volt = %d\n", id_volt);
			id_voltage_read = id_volt;
		}

		if ((sizeof(g_battery_id_voltage) / sizeof(int)) != TOTAL_BATTERY_NUMBER) 
		{
			bm_print(BMLOG_INFO_LEVEL, "CEI(K)=> [fgauge_get_profile_id]error! voltage range incorrect!\n");
		...
		bm_print(BMLOG_INFO_LEVEL, "CEI(K)=> [fgauge_get_profile_id]batt_init_done = %d, g_fg_battery_id = %d\n", batt_init_done, g_fg_battery_id);
	...
	if((customer_project_id == 1) || (customer_project_id == 3))
	{
		if (g_fg_battery_id == 0)
			battery_id_invalid = 1;
	}

	bm_print(BMLOG_INFO_LEVEL, "[fgauge_get_profile_id]Battery id (%d)\n, id_volt=%d, battery_id_invalid=%d", g_fg_battery_id, id_volt, battery_id_invalid);
#endif
//CEI comments, multi bat E
}
...
void get_profile_id_and_volt(int *batt_id, int *batt_id_volt)
{
	...
	*batt_id_volt = 0;
	...
	//ret = IMM_GetOneChannelValue_Cali(BATTERY_ID_CHANNEL_NUM, &id_volt);
	if (ret != 0)
		bm_print(BMLOG_INFO_LEVEL, "[get_profile_id_and_volt]CEI(K)=> id_volt read fail\n");
	else
	{
			bm_print(BMLOG_INFO_LEVEL, "[get_profile_id_and_volt]CEI(K)=> id_volt = %d, swocv_rtc= %d, hwocv_rtc= %d, embedded_battery= %d\n", id_volt, batt_meter_cust_data.difference_swocv_rtc, batt_meter_cust_data.difference_hwocv_rtc, batt_meter_cust_data.embedded_battery);
	}

	*batt_id_volt = id_volt;

	if ((sizeof(g_battery_id_voltage) / sizeof(signed int)) != TOTAL_BATTERY_NUMBER) {
		bm_print(BMLOG_INFO_LEVEL, "[get_profile_id_and_volt]CEI(K)=> error! voltage range incorrect!\n");
		return;
	}
	...
	bm_print(BMLOG_INFO_LEVEL, "CEI(K) [%s], g_fg_battery_id %d, id_volt %d\n\r", __func__, fg_battery_id, id_volt);
...
//CEI comments, multi bat S
	...
	if(customer_project_id == 1)
	{
		batt_meter_table_cust_data.battery_profile_t0_size = sizeof(battery_profile_t0_SM21[g_fg_battery_id]) / sizeof(BATTERY_PROFILE_STRUCT);
		memcpy(&batt_meter_table_cust_data.battery_profile_t0, &battery_profile_t0_SM21[g_fg_battery_id], sizeof(battery_profile_temperature_SM21));

		batt_meter_table_cust_data.battery_profile_t1_size = sizeof(battery_profile_t1_SM21[g_fg_battery_id]) / sizeof(BATTERY_PROFILE_STRUCT);
		memcpy(&batt_meter_table_cust_data.battery_profile_t1, &battery_profile_t1_SM21[g_fg_battery_id], sizeof(battery_profile_temperature_SM21));

		batt_meter_table_cust_data.battery_profile_t2_size = sizeof(battery_profile_t2_SM21[g_fg_battery_id]) / sizeof(BATTERY_PROFILE_STRUCT);
		memcpy(&batt_meter_table_cust_data.battery_profile_t2, &battery_profile_t2_SM21[g_fg_battery_id], sizeof(battery_profile_temperature_SM21));

		batt_meter_table_cust_data.battery_profile_t3_size = sizeof(battery_profile_t3_SM21[g_fg_battery_id]) / sizeof(BATTERY_PROFILE_STRUCT);
		memcpy(&batt_meter_table_cust_data.battery_profile_t3, &battery_profile_t3_SM21[g_fg_battery_id], sizeof(battery_profile_temperature_SM21));

		batt_meter_table_cust_data.r_profile_t0_size = sizeof(r_profile_t0_SM21[g_fg_battery_id]) / sizeof(R_PROFILE_STRUCT);
		memcpy(&batt_meter_table_cust_data.r_profile_t0, &r_profile_t0_SM21[g_fg_battery_id], sizeof(r_profile_temperature_SM21));
	
		batt_meter_table_cust_data.r_profile_t1_size = sizeof(r_profile_t1_SM21[g_fg_battery_id]) / sizeof(R_PROFILE_STRUCT);
		memcpy(&batt_meter_table_cust_data.r_profile_t1, &r_profile_t1_SM21[g_fg_battery_id], sizeof(r_profile_temperature_SM21));
	
		batt_meter_table_cust_data.r_profile_t2_size = sizeof(r_profile_t2_SM21[g_fg_battery_id]) / sizeof(R_PROFILE_STRUCT);
		memcpy(&batt_meter_table_cust_data.r_profile_t2, &r_profile_t2_SM21[g_fg_battery_id], sizeof(r_profile_temperature_SM21));
	
		batt_meter_table_cust_data.r_profile_t3_size = sizeof(r_profile_t3_SM21[g_fg_battery_id]) / sizeof(R_PROFILE_STRUCT);
		memcpy(&batt_meter_table_cust_data.r_profile_t3, &r_profile_t3_SM21[g_fg_battery_id], sizeof(r_profile_temperature_SM21));

	}
	else if(customer_project_id == 3)
	{
		batt_meter_table_cust_data.battery_profile_t0_size = sizeof(battery_profile_t0_SM11L[g_fg_battery_id]) / sizeof(BATTERY_PROFILE_STRUCT);
		memcpy(&batt_meter_table_cust_data.battery_profile_t0, &battery_profile_t0_SM11L[g_fg_battery_id], sizeof(battery_profile_temperature_SM11L));

		batt_meter_table_cust_data.battery_profile_t1_size = sizeof(battery_profile_t1_SM11L[g_fg_battery_id]) / sizeof(BATTERY_PROFILE_STRUCT);
		memcpy(&batt_meter_table_cust_data.battery_profile_t1, &battery_profile_t1_SM11L[g_fg_battery_id], sizeof(battery_profile_temperature_SM11L));

		batt_meter_table_cust_data.battery_profile_t2_size = sizeof(battery_profile_t2_SM11L[g_fg_battery_id]) / sizeof(BATTERY_PROFILE_STRUCT);
		memcpy(&batt_meter_table_cust_data.battery_profile_t2, &battery_profile_t2_SM11L[g_fg_battery_id], sizeof(battery_profile_temperature_SM11L));

		batt_meter_table_cust_data.battery_profile_t3_size = sizeof(battery_profile_t3_SM11L[g_fg_battery_id]) / sizeof(BATTERY_PROFILE_STRUCT);
		memcpy(&batt_meter_table_cust_data.battery_profile_t3, &battery_profile_t3_SM11L[g_fg_battery_id], sizeof(battery_profile_temperature_SM11L));

		batt_meter_table_cust_data.r_profile_t0_size = sizeof(r_profile_t0_SM11L[g_fg_battery_id]) / sizeof(R_PROFILE_STRUCT);
		memcpy(&batt_meter_table_cust_data.r_profile_t0, &r_profile_t0_SM11L[g_fg_battery_id], sizeof(r_profile_temperature_SM11L));
	
		batt_meter_table_cust_data.r_profile_t1_size = sizeof(r_profile_t1_SM11L[g_fg_battery_id]) / sizeof(R_PROFILE_STRUCT);
		memcpy(&batt_meter_table_cust_data.r_profile_t1, &r_profile_t1_SM11L[g_fg_battery_id], sizeof(r_profile_temperature_SM11L));
	
		batt_meter_table_cust_data.r_profile_t2_size = sizeof(r_profile_t2_SM11L[g_fg_battery_id]) / sizeof(R_PROFILE_STRUCT);
		memcpy(&batt_meter_table_cust_data.r_profile_t2, &r_profile_t2_SM11L[g_fg_battery_id], sizeof(r_profile_temperature_SM11L));
	
		batt_meter_table_cust_data.r_profile_t3_size = sizeof(r_profile_t3_SM11L[g_fg_battery_id]) / sizeof(R_PROFILE_STRUCT);
		memcpy(&batt_meter_table_cust_data.r_profile_t3, &r_profile_t3_SM11L[g_fg_battery_id], sizeof(r_profile_temperature_SM11L));
	}
	else
	{
		batt_meter_table_cust_data.battery_profile_t0_size = sizeof(battery_profile_t0_SM11[g_fg_battery_id]) / sizeof(BATTERY_PROFILE_STRUCT);
		memcpy(&batt_meter_table_cust_data.battery_profile_t0, &battery_profile_t0_SM11[g_fg_battery_id], sizeof(battery_profile_temperature_SM11));

		batt_meter_table_cust_data.battery_profile_t1_size = sizeof(battery_profile_t1_SM11[g_fg_battery_id]) / sizeof(BATTERY_PROFILE_STRUCT);
		memcpy(&batt_meter_table_cust_data.battery_profile_t1, &battery_profile_t1_SM11[g_fg_battery_id], sizeof(battery_profile_temperature_SM11));

		batt_meter_table_cust_data.battery_profile_t2_size = sizeof(battery_profile_t2_SM11[g_fg_battery_id]) / sizeof(BATTERY_PROFILE_STRUCT);
		memcpy(&batt_meter_table_cust_data.battery_profile_t2, &battery_profile_t2_SM11[g_fg_battery_id], sizeof(battery_profile_temperature_SM11));

		batt_meter_table_cust_data.battery_profile_t3_size = sizeof(battery_profile_t3_SM11[g_fg_battery_id]) / sizeof(BATTERY_PROFILE_STRUCT);
		memcpy(&batt_meter_table_cust_data.battery_profile_t3, &battery_profile_t3_SM11[g_fg_battery_id], sizeof(battery_profile_temperature_SM11));

		batt_meter_table_cust_data.r_profile_t0_size = sizeof(r_profile_t0_SM11[g_fg_battery_id]) / sizeof(R_PROFILE_STRUCT);
		memcpy(&batt_meter_table_cust_data.r_profile_t0, &r_profile_t0_SM11[g_fg_battery_id], sizeof(r_profile_temperature_SM11));
	
		batt_meter_table_cust_data.r_profile_t1_size = sizeof(r_profile_t1_SM11[g_fg_battery_id]) / sizeof(R_PROFILE_STRUCT);
		memcpy(&batt_meter_table_cust_data.r_profile_t1, &r_profile_t1_SM11[g_fg_battery_id], sizeof(r_profile_temperature_SM11));
	
		batt_meter_table_cust_data.r_profile_t2_size = sizeof(r_profile_t2_SM11[g_fg_battery_id]) / sizeof(R_PROFILE_STRUCT);
		memcpy(&batt_meter_table_cust_data.r_profile_t2, &r_profile_t2_SM11[g_fg_battery_id], sizeof(r_profile_temperature_SM11));
	
		batt_meter_table_cust_data.r_profile_t3_size = sizeof(r_profile_t3_SM11[g_fg_battery_id]) / sizeof(R_PROFILE_STRUCT);
		memcpy(&batt_meter_table_cust_data.r_profile_t3, &r_profile_t3_SM11[g_fg_battery_id], sizeof(r_profile_temperature_SM11));
	}
...
//CEI comments, multi bat S
	...
	if(customer_project_id == 1)
	{
		batt_meter_cust_data.q_max_pos_50 = g_Q_MAX_POS_50_SM21[g_fg_battery_id];
		batt_meter_cust_data.q_max_pos_25 = g_Q_MAX_POS_25_SM21[g_fg_battery_id];
		batt_meter_cust_data.q_max_pos_0 = g_Q_MAX_POS_0_SM21[g_fg_battery_id];
		batt_meter_cust_data.q_max_neg_10 = g_Q_MAX_NEG_10_SM21[g_fg_battery_id];
		batt_meter_cust_data.q_max_pos_50_h_current = g_Q_MAX_POS_50_H_CURRENT_SM21[g_fg_battery_id];
		batt_meter_cust_data.q_max_pos_25_h_current = g_Q_MAX_POS_25_H_CURRENT_SM21[g_fg_battery_id];
		batt_meter_cust_data.q_max_pos_0_h_current = g_Q_MAX_POS_0_H_CURRENT_SM21[g_fg_battery_id];
		batt_meter_cust_data.q_max_neg_10_h_current = g_Q_MAX_NEG_10_H_CURRENT_SM21[g_fg_battery_id];
//CEI comments, soft charge 3.0 start
		batt_meter_cust_data.q_max_pos_50_lcv = g_Q_MAX_POS_50_LCV_SM21[g_fg_battery_id];
		batt_meter_cust_data.q_max_pos_25_lcv = g_Q_MAX_POS_25_LCV_SM21[g_fg_battery_id];
		batt_meter_cust_data.q_max_pos_0_lcv = g_Q_MAX_POS_0_LCV_SM21[g_fg_battery_id];
		batt_meter_cust_data.q_max_neg_10_lcv = g_Q_MAX_NEG_10_LCV_SM21[g_fg_battery_id];
		batt_meter_cust_data.q_max_pos_50_h_current_lcv = g_Q_MAX_POS_50_H_CURRENT_LCV_SM21[g_fg_battery_id];
		batt_meter_cust_data.q_max_pos_25_h_current_lcv = g_Q_MAX_POS_25_H_CURRENT_LCV_SM21[g_fg_battery_id];
		batt_meter_cust_data.q_max_pos_0_h_current_lcv = g_Q_MAX_POS_0_H_CURRENT_LCV_SM21[g_fg_battery_id];
		batt_meter_cust_data.q_max_neg_10_h_current_lcv = g_Q_MAX_NEG_10_H_CURRENT_LCV_SM21[g_fg_battery_id];
//CEI comments, soft charge 3.0 end
	}
	else if(customer_project_id == 3)
	{
		batt_meter_cust_data.q_max_pos_50 = g_Q_MAX_POS_50_SM11L[g_fg_battery_id];
		batt_meter_cust_data.q_max_pos_25 = g_Q_MAX_POS_25_SM11L[g_fg_battery_id];
		batt_meter_cust_data.q_max_pos_0 = g_Q_MAX_POS_0_SM11L[g_fg_battery_id];
		batt_meter_cust_data.q_max_neg_10 = g_Q_MAX_NEG_10_SM11L[g_fg_battery_id];
		batt_meter_cust_data.q_max_pos_50_h_current = g_Q_MAX_POS_50_H_CURRENT_SM11L[g_fg_battery_id];
		batt_meter_cust_data.q_max_pos_25_h_current = g_Q_MAX_POS_25_H_CURRENT_SM11L[g_fg_battery_id];
		batt_meter_cust_data.q_max_pos_0_h_current = g_Q_MAX_POS_0_H_CURRENT_SM11L[g_fg_battery_id];
		batt_meter_cust_data.q_max_neg_10_h_current = g_Q_MAX_NEG_10_H_CURRENT_SM11L[g_fg_battery_id];
//CEI comments, soft charge 3.0 start
		batt_meter_cust_data.q_max_pos_50_lcv = g_Q_MAX_POS_50_LCV_SM11L[g_fg_battery_id];
		batt_meter_cust_data.q_max_pos_25_lcv = g_Q_MAX_POS_25_LCV_SM11L[g_fg_battery_id];
		batt_meter_cust_data.q_max_pos_0_lcv = g_Q_MAX_POS_0_LCV_SM11L[g_fg_battery_id];
		batt_meter_cust_data.q_max_neg_10_lcv = g_Q_MAX_NEG_10_LCV_SM11L[g_fg_battery_id];
		batt_meter_cust_data.q_max_pos_50_h_current_lcv = g_Q_MAX_POS_50_H_CURRENT_LCV_SM11L[g_fg_battery_id];
		batt_meter_cust_data.q_max_pos_25_h_current_lcv = g_Q_MAX_POS_25_H_CURRENT_LCV_SM11L[g_fg_battery_id];
		batt_meter_cust_data.q_max_pos_0_h_current_lcv = g_Q_MAX_POS_0_H_CURRENT_LCV_SM11L[g_fg_battery_id];
		batt_meter_cust_data.q_max_neg_10_h_current_lcv = g_Q_MAX_NEG_10_H_CURRENT_LCV_SM11L[g_fg_battery_id];
//CEI comments, soft charge 3.0 end
	}
	else
	{
		batt_meter_cust_data.q_max_pos_50 = g_Q_MAX_POS_50_SM11[g_fg_battery_id];
		batt_meter_cust_data.q_max_pos_25 = g_Q_MAX_POS_25_SM11[g_fg_battery_id];
		batt_meter_cust_data.q_max_pos_0 = g_Q_MAX_POS_0_SM11[g_fg_battery_id];
		batt_meter_cust_data.q_max_neg_10 = g_Q_MAX_NEG_10_SM11[g_fg_battery_id];
		batt_meter_cust_data.q_max_pos_50_h_current = g_Q_MAX_POS_50_H_CURRENT_SM11[g_fg_battery_id];
		batt_meter_cust_data.q_max_pos_25_h_current = g_Q_MAX_POS_25_H_CURRENT_SM11[g_fg_battery_id];
		batt_meter_cust_data.q_max_pos_0_h_current = g_Q_MAX_POS_0_H_CURRENT_SM11[g_fg_battery_id];
		batt_meter_cust_data.q_max_neg_10_h_current = g_Q_MAX_NEG_10_H_CURRENT_SM11[g_fg_battery_id];
//CEI comments, soft charge 3.0 start
		batt_meter_cust_data.q_max_pos_50_lcv = g_Q_MAX_POS_50_LCV_SM11[g_fg_battery_id];
		batt_meter_cust_data.q_max_pos_25_lcv = g_Q_MAX_POS_25_LCV_SM11[g_fg_battery_id];
		batt_meter_cust_data.q_max_pos_0_lcv = g_Q_MAX_POS_0_LCV_SM11[g_fg_battery_id];
		batt_meter_cust_data.q_max_neg_10_lcv = g_Q_MAX_NEG_10_LCV_SM11[g_fg_battery_id];
		batt_meter_cust_data.q_max_pos_50_h_current_lcv = g_Q_MAX_POS_50_H_CURRENT_LCV_SM11[g_fg_battery_id];
		batt_meter_cust_data.q_max_pos_25_h_current_lcv = g_Q_MAX_POS_25_H_CURRENT_LCV_SM11[g_fg_battery_id];
		batt_meter_cust_data.q_max_pos_0_h_current_lcv = g_Q_MAX_POS_0_H_CURRENT_LCV_SM11[g_fg_battery_id];
		batt_meter_cust_data.q_max_neg_10_h_current_lcv = g_Q_MAX_NEG_10_H_CURRENT_LCV_SM11[g_fg_battery_id];
//CEI comments, soft charge 3.0 end
	}	

#ifdef LE_DEBUG_BATT_PROFILE2
		bm_print(BMLOG_INFO_LEVEL, "CEI(K)=> batt_meter_cust_data.q_max_pos_50 = %d, batt_meter_cust_data.q_max_pos_25 = %d, batt_meter_cust_data.q_max_pos_0 = %d, batt_meter_cust_data.q_max_neg_10 = %d\n",
...
//CAR_TUNE_VAL tuning
	...
	bm_err("LE(K)=> %s Get car_tune_value %d\n", __func__, batt_meter_cust_data.car_tune_value);
//CEI comment end//
...
//parameter setting
	...
	if(customer_project_id == 3)
		batt_meter_cust_data.difference_swocv_rtc = DIFFERENCE_SWOCV_RTC_SM11L;
	else
		batt_meter_cust_data.difference_swocv_rtc = DIFFERENCE_SWOCV_RTC_SM11_SM21;
//CEI comment end
...
//CEI comments, multi bat S
	...
	if(customer_project_id == 1)
		batt_temperature_table = &Batt_Temperature_Table_SM21[g_fg_battery_id][0];
	else if(customer_project_id == 3)
		batt_temperature_table = &Batt_Temperature_Table_SM11L[g_fg_battery_id][0];
	else
		batt_temperature_table = &Batt_Temperature_Table_SM11[g_fg_battery_id][0];
//CEI comments, multi bat E
...
//CEI comments, multi bat S
	...
	if(customer_project_id == 1)
	{
		if (temperature <= TEMPERATURE_T1) {
			low_temperature = (-10);
			tmp_Q_max_1 = g_Q_MAX_NEG_10_SM21[g_fg_battery_id];
			high_temperature = TEMPERATURE_T1;
			tmp_Q_max_2 = g_Q_MAX_POS_0_SM21[g_fg_battery_id];
			...
			low_temperature = TEMPERATURE_T1;
			tmp_Q_max_1 = g_Q_MAX_POS_0_SM21[g_fg_battery_id];
			high_temperature = TEMPERATURE_T2;
			tmp_Q_max_2 = g_Q_MAX_POS_25_SM21[g_fg_battery_id];
			...
			low_temperature = TEMPERATURE_T2;
			tmp_Q_max_1 = g_Q_MAX_POS_25_SM21[g_fg_battery_id];
			high_temperature = TEMPERATURE_T3;
			tmp_Q_max_2 = g_Q_MAX_POS_50_SM21[g_fg_battery_id];
	...
	else if(customer_project_id == 3)
	{
		if (temperature <= TEMPERATURE_T1) {
			low_temperature = (-10);
			tmp_Q_max_1 = g_Q_MAX_NEG_10_SM11L[g_fg_battery_id];
			high_temperature = TEMPERATURE_T1;
			tmp_Q_max_2 = g_Q_MAX_POS_0_SM11L[g_fg_battery_id];
			...
			low_temperature = TEMPERATURE_T1;
			tmp_Q_max_1 = g_Q_MAX_POS_0_SM11L[g_fg_battery_id];
			high_temperature = TEMPERATURE_T2;
			tmp_Q_max_2 = g_Q_MAX_POS_25_SM11L[g_fg_battery_id];
			...
			low_temperature = TEMPERATURE_T2;
			tmp_Q_max_1 = g_Q_MAX_POS_25_SM11L[g_fg_battery_id];
			high_temperature = TEMPERATURE_T3;
			tmp_Q_max_2 = g_Q_MAX_POS_50_SM11L[g_fg_battery_id];
	...
	else
	{
		if (temperature <= TEMPERATURE_T1) {
			low_temperature = (-10);
			tmp_Q_max_1 = g_Q_MAX_NEG_10_SM11[g_fg_battery_id];
			high_temperature = TEMPERATURE_T1;
			tmp_Q_max_2 = g_Q_MAX_POS_0_SM11[g_fg_battery_id];
			...
			low_temperature = TEMPERATURE_T1;
			tmp_Q_max_1 = g_Q_MAX_POS_0_SM11[g_fg_battery_id];
			high_temperature = TEMPERATURE_T2;
			tmp_Q_max_2 = g_Q_MAX_POS_25_SM11[g_fg_battery_id];
			...
			low_temperature = TEMPERATURE_T2;
			tmp_Q_max_1 = g_Q_MAX_POS_25_SM11[g_fg_battery_id];
			high_temperature = TEMPERATURE_T3;
			tmp_Q_max_2 = g_Q_MAX_POS_50_SM11[g_fg_battery_id];
...
//CEI comments, multi bat S
	...
	if(customer_project_id == 1)
	{
		if (temperature <= TEMPERATURE_T1) {
			low_temperature = (-10);
			tmp_Q_max_1 = g_Q_MAX_NEG_10_H_CURRENT_SM21[g_fg_battery_id];
			high_temperature = TEMPERATURE_T1;
			tmp_Q_max_2 = g_Q_MAX_POS_0_H_CURRENT_SM21[g_fg_battery_id];
			...
			low_temperature = TEMPERATURE_T1;
			tmp_Q_max_1 = g_Q_MAX_POS_0_H_CURRENT_SM21[g_fg_battery_id];
			high_temperature = TEMPERATURE_T2;
			tmp_Q_max_2 = g_Q_MAX_POS_25_H_CURRENT_SM21[g_fg_battery_id];
			...
			low_temperature = TEMPERATURE_T2;
			tmp_Q_max_1 = g_Q_MAX_POS_25_H_CURRENT_SM21[g_fg_battery_id];
			high_temperature = TEMPERATURE_T3;
			tmp_Q_max_2 = g_Q_MAX_POS_50_H_CURRENT_SM21[g_fg_battery_id];
	...
	else if(customer_project_id == 3)
	{
		if (temperature <= TEMPERATURE_T1) {
			low_temperature = (-10);
			tmp_Q_max_1 = g_Q_MAX_NEG_10_H_CURRENT_SM11L[g_fg_battery_id];
			high_temperature = TEMPERATURE_T1;
			tmp_Q_max_2 = g_Q_MAX_POS_0_H_CURRENT_SM11L[g_fg_battery_id];
			...
			low_temperature = TEMPERATURE_T1;
			tmp_Q_max_1 = g_Q_MAX_POS_0_H_CURRENT_SM11L[g_fg_battery_id];
			high_temperature = TEMPERATURE_T2;
			tmp_Q_max_2 = g_Q_MAX_POS_25_H_CURRENT_SM11L[g_fg_battery_id];
			...
			low_temperature = TEMPERATURE_T2;
			tmp_Q_max_1 = g_Q_MAX_POS_25_H_CURRENT_SM11L[g_fg_battery_id];
			high_temperature = TEMPERATURE_T3;
			tmp_Q_max_2 = g_Q_MAX_POS_50_H_CURRENT_SM11L[g_fg_battery_id];
	...
	else
	{
		if (temperature <= TEMPERATURE_T1) {
			low_temperature = (-10);
			tmp_Q_max_1 = g_Q_MAX_NEG_10_H_CURRENT_SM11[g_fg_battery_id];
			high_temperature = TEMPERATURE_T1;
			tmp_Q_max_2 = g_Q_MAX_POS_0_H_CURRENT_SM11[g_fg_battery_id];
			...
			low_temperature = TEMPERATURE_T1;
			tmp_Q_max_1 = g_Q_MAX_POS_0_H_CURRENT_SM11[g_fg_battery_id];
			high_temperature = TEMPERATURE_T2;
			tmp_Q_max_2 = g_Q_MAX_POS_25_H_CURRENT_SM11[g_fg_battery_id];
			...
			low_temperature = TEMPERATURE_T2;
			tmp_Q_max_1 = g_Q_MAX_POS_25_H_CURRENT_SM11[g_fg_battery_id];
			high_temperature = TEMPERATURE_T3;
			tmp_Q_max_2 = g_Q_MAX_POS_50_H_CURRENT_SM11[g_fg_battery_id];
...
//CEI comments, multi bat S
	bm_err("[BATTERY] q_max, -10=%d, 0=%d, 25=%d, 50=%d\n",
		batt_meter_cust_data.q_max_neg_10, batt_meter_cust_data.q_max_pos_0,
		batt_meter_cust_data.q_max_pos_25, batt_meter_cust_data.q_max_pos_50);
//CEI comments, multi bat E
...
signed int battery_meter_get_design_qmax(void)
{
	...
	if(customer_project_id == 1)
		return g_Q_MAX_POS_50_SM21[g_fg_battery_id];
	else if(customer_project_id == 3)
		return g_Q_MAX_POS_50_SM11L[g_fg_battery_id];
	else
		return g_Q_MAX_POS_50_SM11[g_fg_battery_id];
}
...
static ssize_t show_BMTbat_vol(struct device *dev,struct device_attribute *attr, char *buf)
{
	...
	bm_err("[show_BMTbat_vol]CEI(K)=> BMT_status.bat_vol = %u\n", temp_bat_vol);
...
{
	bm_err("[show_bypass_bcct]CEI(K)=> bypass_bcct = %d\n", bypass_bcct);
	return sprintf(buf, "%d\n", bypass_bcct);
}
static ssize_t store_bypass_bcct(struct device *dev,struct device_attribute *attr, const char*buf, size_t size)
{
	if ( 0 == kstrtoint(buf, 10, &bypass_bcct) ) {
		bm_err("[store_bypass_bcct]CEI(K)=> bypass_bcct = %d\n", bypass_bcct);
	} else {
		bm_err("[store_bypass_bcct]CEI(K)=> format error!\n");
	}
		return size;	
	}
...
//CEI comments, car_tune_value debug use S
static ssize_t show_car_tune_value(struct device *dev,struct device_attribute *attr, char *buf)
{
	bm_err("%s => car_tune_value %d\n", __func__, batt_meter_cust_data.car_tune_value);
	
	return sprintf(buf, "car_tune_value=%d \n", batt_meter_cust_data.car_tune_value);
}
static ssize_t store_car_tune_value(struct device *dev,struct device_attribute *attr, const char*buf, size_t size)
{
	return size;
}
static DEVICE_ATTR(car_tune_value, 0664, show_car_tune_value, store_car_tune_value);
//CEI comments, car_tune_value debug use E
...
//CEI comment start//
//CAR_TUNE_VAL tuning
extern int nvram_car_tune;
//CEI comment end//

//CEI comment start//
//battery health(aging factor)
extern const int g_charge_full_design_SM11; //mAh
extern const int g_charge_full_design_SM21; //mAh
extern const int g_charge_full_design_SM11L; //mAh
extern int g_age_counter;
extern int g_age_mtkcalc;
extern int g_age_avgmiscta;
...
//CEI comment start//
//This ioctl cmd changes present car_tune_value, need to leave a log
			//bm_notice("[fg_res] NVRAM_CAR_TUNE_VALUE = %d\n", NVRAM_CAR_TUNE_VALUE);
			bm_err("[fg_res] LE(K)=> NVRAM_CAR_TUNE_VALUE = %d !!! (WARNING)\n", NVRAM_CAR_TUNE_VALUE);
			nvram_car_tune = NVRAM_CAR_TUNE_VALUE;
//CEI comment end//
...
//battery health (aging factor)
		case FG_DAEMON_CMD_SET_AGING_FACTOR:
		{
			...
			bm_err("[fg_res] aging_factor_tmp = %d\n", aging_factor_tmp);
			gFG_aging_factor_1 = aging_factor_tmp;

			if( (aging_factor_tmp >= 0) && (aging_factor_tmp <= 100) )
			{
				int tmp_age_mtkcalc = 0;

				if(customer_project_id == 1)
				{
					tmp_age_mtkcalc = (aging_factor_tmp * g_Q_MAX_POS_25_SM21[g_fg_battery_id]) / 100;
					if (tmp_age_mtkcalc > g_charge_full_design_SM21)
						tmp_age_mtkcalc = g_charge_full_design_SM21;
				}
				else if(customer_project_id == 3)
				{
					tmp_age_mtkcalc = (aging_factor_tmp * g_Q_MAX_POS_25_SM11L[g_fg_battery_id]) / 100;
					if (tmp_age_mtkcalc > g_charge_full_design_SM11L)
						tmp_age_mtkcalc = g_charge_full_design_SM11L;
				}
				else
				{
					tmp_age_mtkcalc = (aging_factor_tmp * g_Q_MAX_POS_25_SM11[g_fg_battery_id]) / 100;
					if (tmp_age_mtkcalc > g_charge_full_design_SM11)
						tmp_age_mtkcalc = g_charge_full_design_SM11;
				}

				g_age_mtkcalc = tmp_age_mtkcalc;
				g_age_counter = g_age_counter + 1;
				if(customer_project_id == 1)
					bm_err("[AGING] CEI(K)=> g_Q_MAX_POS_25_SM21[%d]=%d, aging_factor_tmp=%d, g_age_mtkcalc=%d, g_age_counter=%d\n",
					g_fg_battery_id, g_Q_MAX_POS_25_SM21[g_fg_battery_id], aging_factor_tmp, g_age_mtkcalc, g_age_counter);
				else if(customer_project_id == 3)
					bm_err("[AGING] CEI(K)=> g_Q_MAX_POS_25_SM11L[%d]=%d, aging_factor_tmp=%d, g_age_mtkcalc=%d, g_age_counter=%d\n",
					g_fg_battery_id, g_Q_MAX_POS_25_SM11L[g_fg_battery_id], aging_factor_tmp, g_age_mtkcalc, g_age_counter);
				else
					bm_err("[AGING] CEI(K)=> g_Q_MAX_POS_25_SM11[%d]=%d, aging_factor_tmp=%d, g_age_mtkcalc=%d, g_age_counter=%d\n",
					g_fg_battery_id, g_Q_MAX_POS_25_SM11[g_fg_battery_id], aging_factor_tmp, g_age_mtkcalc, g_age_counter);
			}
		}
		break;
//CEI comment end
