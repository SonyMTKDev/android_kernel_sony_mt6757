...
static void RD_I2C(unsigned char slvadr, unsigned char size, unsigned char *dat, unsigned char *a_pRecvData)
{
...
static void WR_I2C(unsigned char slvadr, unsigned short int size, unsigned char *dat)
{
...
static void setVCMPos(unsigned short target)
{
...
/* **** Download the data */
/* ***************************************************** */
static void download(unsigned short int u16_type, unsigned short int u16_coef_type)
{
 ...
 *****************************************************/
static void func_PROGRAM_DOWNLOAD(unsigned char type)
{
	unsigned short sts;
...
/*****************************************************
  **** COEF Download
*****************************************************/
static void func_COEF_DOWNLOAD(unsigned char coef_type)
{
	unsigned short u16_dat;																	
...
static unsigned short convertAF_DAC(short NRMHAL)
{
	int trans, pos;

	if((NRMHAL > 16376) && (NRMHAL < 65536))
		trans = NRMHAL - 65535;
	else
		trans = NRMHAL;
...
static inline int moveAF(unsigned long a_u4Position)
{
	...
	if (*g_pAF_Opened == 1) {
		...
		*g_pAF_Opened = 2;
		...
		initPos = convertAF_DAC(BU64747GWZAF_getHall());
		LOG_INF("Init Position = %d", initPos);
...
int BU64747GWZAF_SetI2Cclient(struct i2c_client *pstAF_I2Cclient, spinlock_t *pAF_SpinLock, int *pAF_Opened)
{
