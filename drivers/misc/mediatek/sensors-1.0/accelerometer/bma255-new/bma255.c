...
//[SM11L] To change direction for HW PDP.
...
#define BMA255_REG_SLOPE_DUR	0x27
#define BMA255_SLOPE_DUR_MAX	3
#define SLOPE_THRES			0x1e		// 7.81mg * 30 = 234.3mg
#define SLOPE_DURATION			0x00		// one sample
#define BMA255_REG_INT_STATUS		0x09
//[SM11L] To change direction for HW PDP
#define SM11L_HW_PDP		        0
#define CUSTOMER_PROJECT_SM11L  3
...
#endif
    u8 config_slope_thresh;
    u8 config_slope_duration;
};
#if !defined(CONFIG_HAS_EARLYSUSPEND)    
...
static int bma255_setup_any_motion_detection(struct bma255_i2c_data *obj, bool enable)
{
	...
	if (obj->config_slope_thresh)
		cfg.slope_thres = obj->config_slope_thresh;
	else
		cfg.slope_thres = SLOPE_THRES;

	if (obj->config_slope_duration)
		cfg.slope_duration = obj->config_slope_duration;
	else
		cfg.slope_duration = SLOPE_DURATION;
	...
	if (rc)
		goto err;
	mdelay(1);
	rc = bma_i2c_write_block(obj->client, BMA255_REG_SLOPE_DUR, &cfg.slope_duration, 1);
	if (rc)
		goto err;
	mdelay(1);
	...
	if (rc)
		goto err;
	rc = bma_i2c_read_block(obj->client, BMA255_REG_SLOPE_DUR, &cfg.slope_duration, 1);
	if (rc)
		goto err;
	dev_info(&obj->client->dev,
		"%s, MODE %02x  INTEN %02x  INTMAP %02x  INTCTRL %02x  THRES %02x DUR %02x\n",
		__func__, cfg.mode, cfg.int_en, cfg.intmap, cfg.int_out_ctrl,
		cfg.slope_thres, cfg.slope_duration);
err:
	if (rc)
...
static int print_motion_config(struct motion_config *cfg, char *buf, size_t size)
{
	return scnprintf(buf, size,
		"MODE %02x  INTEN %02x  INTMAP %02x  INTCTRL %02x  THRES %02x  DUR %02x  INTSTATUS %02x\n",
		cfg->mode, cfg->int_en, cfg->intmap, cfg->int_out_ctrl, cfg->slope_thres, cfg->slope_duration, cfg->int_status);
}
...
static ssize_t store_motion_config(struct device_driver *ddri, const char *buf, size_t count)
{
	struct bma255_i2c_data *obj = obj_i2c_data;
	int rc;
	int enable, thresh, dur;

	rc = sscanf(buf, "%i %i %i", &enable, &thresh, &dur);

	switch (rc) {
	case 3:
		dev_info(&obj->client->dev, "%s: duration %d\n",
				__func__, dur);
		if (dur > BMA255_SLOPE_DUR_MAX)
			dur = BMA255_SLOPE_DUR_MAX;
		obj->config_slope_duration = dur;
	case 2:
		dev_info(&obj->client->dev, "%s: thresh %d\n",
				__func__, thresh);
		obj->config_slope_thresh = thresh;
	case 1:
		dev_info(&obj->client->dev, "%s: enable %d\n",
				__func__, enable);
		rc = vote_op_mode(obj->client, enable == 1,
				REQ_SHAKE_SENSOR);
		break;
	default:
		rc = -EINVAL;
...
static int gsensor_batch(int flag, int64_t samplingPeriodNs, int64_t maxBatchReportLatencyNs)
{
	...
	GSE_LOG("bma255 acc set delay = (%d) ok.\n", value);
	return bma255_set_delay(samplingPeriodNs);
}
...
//tad3sgh add --
	...
	//[SM11L] To change direction for HW PDP(default:1,PDP:2)
	if((get_cei_customer_project_id() == CUSTOMER_PROJECT_SM11L) && (get_cei_hw_id() == SM11L_HW_PDP))
	{
		printk("%s, project_id = %d, hw_id = %d\n", __func__, CUSTOMER_PROJECT_SM11L, SM11L_HW_PDP);
		obj->hw->direction= 2;
	}

        printk("%s, obj->hw->direction = %d\n", __func__, obj->hw->direction);
