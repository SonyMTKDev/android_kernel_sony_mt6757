...
#include "cei_hw_id.h"	/*Add for Android N to O one image*/
...
unsigned int synaptics_tpd_rst_gpio_number = 0;
unsigned int synaptics_tpd_int_gpio_number = 1;

#ifdef TP_DYNAMIC_LOG
unsigned int synaptics_tp_dynamic_log_level = TP_LOG_LEVEL_INFO;
#endif

static const struct of_device_id synaptics_dt_match[] = {
	{.compatible = "mediatek,cap_touch_sm21"},
	{},
};
...
static int of_get_synaptic_platform_data(struct device *dev)
{
	...
	TP_LOGI("tpd_rst_gpio_number = %d\n", synaptics_tpd_rst_gpio_number);
	TP_LOGI("tpd_int_gpio_number = %d\n", synaptics_tpd_int_gpio_number);
...
static int tpd_irq_registration(void)
{
	...
	if (node) {
		...
		ret = request_irq(synaptics_touch_irq, (irq_handler_t) tpd_eint_handler, IRQF_TRIGGER_LOW,
...
rescan_pdt:
	...
	for (page_number = 0; page_number < PAGES_TO_SERVICE; page_number++) {
		for (pdt_entry_addr = PDT_START; pdt_entry_addr > PDT_END;
			...
			TP_LOGI("synaptics_rmi4_query_device pdt_entry_addr = %x\n", pdt_entry_addr);
			retval = synaptics_rmi4_i2c_read(rmi4_data,
					pdt_entry_addr,
...
static ssize_t cei_dynamic_log_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return snprintf(buf, PAGE_SIZE, "%d\n", synaptics_tp_dynamic_log_level);
}
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (kstrtou8(buf, 0, &i) == 0 && i <= TP_LOG_LEVEL_DEBUG) {
		TP_LOGI("dynamic log level (%u) changed to (%u)\n",
			synaptics_tp_dynamic_log_level, i);
		synaptics_tp_dynamic_log_level = i;
...
//==================20161019
	touch_depth_r=touch_irqdepth(synaptics_touch_irq);
	if(touch_depth_r==1){
		enable_irq(synaptics_touch_irq);
...
{
	enum cei_customer_project_type project_id;	/*20170920 Add for Android N to O one image*/
	
	TP_LOGI("init start\n");
	/*20170920 Add for Android N to O one image*/
	project_id = get_cei_customer_project_id();
	TP_LOGI("[Optical]project_id = %d\n", project_id);
	if ( project_id == CUSTOMER_PROJECT_SM21 ) {
		TP_LOGI("[Optical]This is SM21");
	} else {
		TP_LOGI("[Optical]The Project ID is not SM21, exit");
		return 0;
	}
	/*20170920 Add for Android N to O one image*/
