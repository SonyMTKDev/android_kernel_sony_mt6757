...
tcsense_calc:
	...
	/* rounding */
	tc_sense = (1073741824U / vtemp * 250 / 225) << 1;
	if (tc_sense & 0x10000)
...
static int rt5508_adap_coefficent_fix(struct snd_soc_codec *codec)
{
	int i = 0, j = 0, index = 0;
	int ret = 0;
	int64_t x = 0, y = 0, z = 0;
...
out_read:
	...
	x = ret;
	for (i = 0; i < 6; i++) {
		ret = snd_soc_read(codec, RT5508_REG_ADAPTB0 + i);
		ret &= 0xffffff;
		...
		if (ret < 0x800000)
			...
			y = ((int64_t)0xffffff - y) * x;
			z = (int64_t)0x7FFFFF800000 - y;
		}
		z >>= 23;
		ret = z & 0xffffff;
		dev_info(codec->dev, "b factor after 0x%08x\n", ret);
		ret = snd_soc_write(codec, RT5508_REG_ADAPTB0 + i, ret);
		if (ret < 0) {
...
static int rt5508_init_proprietary_setting(struct snd_soc_codec *codec)
{
	...
	ret = rt5508_adap_coefficent_fix(codec);
	if (ret < 0) {
		dev_err(codec->dev, "adap_coefficient_fix fail\n");
		goto out_init_proprietary;
	}
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		if (ret < 0)
			goto out_clk_event;
		ret = snd_soc_read(codec, RT5508_REG_OTPCONF);
		if (ret < 0)
			return ret;
		if (!(ret & 0x20)) {
			dev_warn(codec->dev, "OTP read not ready\n");
			return -EINVAL;
		}
		break;
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		ret = snd_soc_update_bits(codec, RT5508_REG_PILOTNISENSE,
			0x07, 0x04);
		if (ret < 0)
			return ret;
		msleep(25);
		...
		if (ret < 0)
			goto out_boost_event;
		ret = snd_soc_update_bits(codec, RT5508_REG_ECO_CTRL, 0x40, 0x40);
		if (ret < 0)
			goto out_boost_event;
		break;
	case SND_SOC_DAPM_PRE_PMD:
		ret = snd_soc_update_bits(codec, RT5508_REG_ECO_CTRL, 0x40, 0);
		if (ret < 0)
			goto out_boost_event;
		ret = snd_soc_update_bits(codec, RT5508_REG_BST_CONF1,
			0x80, 0x80);
	...
	case SND_SOC_DAPM_POST_PMD:
		...
		msleep(25);
		ret = snd_soc_update_bits(codec, RT5508_REG_CHIPEN,
			RT5508_SPKAMP_ENMASK, RT5508_SPKAMP_ENMASK);
		if (ret < 0)
			goto out_boost_event;
		msleep(15);
		ret = snd_soc_update_bits(codec, RT5508_REG_CHIPEN,
...
static int rt5508_aif_prepare(struct snd_pcm_substream *substream,
...
static const struct snd_soc_dai_ops rt5508_dai_ops = {
	...
	.prepare = rt5508_aif_prepare,
	.set_tdm_slot = rt5508_aif_set_tdm_slot,
};

#define RT5508_RATES SNDRV_PCM_RATE_8000_192000
#define RT5508_FORMATS (SNDRV_PCM_FMTBIT_S16 | SNDRV_PCM_FMTBIT_S18_3LE |\
			    ...
			    const struct i2c_device_id *id)
{
	...
	dev_set_name(chip->dev, "%s", "RT5508_MT");
