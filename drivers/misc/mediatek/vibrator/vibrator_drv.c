...
/**/
// For vibrator voltage tuning
static ssize_t store_vosel_cal(struct device *dev, struct device_attribute *attr, const char *buf, size_t size)
{
	unsigned int vosel, cal;
	int rc = sscanf(buf, "%x,%x", &vosel, &cal);

	if (rc == 2) {
		VIB_DEBUG("%s: Start\n", __func__);
		vibr_vosel_cal_set(vosel, cal);
	} else {
		VIB_DEBUG("%s: EINVAL\n", __func__);
		rc = -EINVAL;
	}
...
static ssize_t show_vosel_cal(struct device *dev, struct device_attribute *attr, char *buf)
{
	unsigned int vosel=0, cal=0;
	int rc = 0;

	vibr_vosel_cal_get(&vosel, &cal);
	rc = scnprintf(buf, PAGE_SIZE, "vosel=0x%x , cal=0x%x\n", vosel, cal);

	return rc;
}
static DEVICE_ATTR(vosel_cal, 0644, show_vosel_cal, store_vosel_cal);
/**/
...
static int vib_mod_init(void)
{
	...
	/**/
	ret = device_create_file(mtk_vibrator.dev, &dev_attr_vosel_cal);
	if (ret)
		VIB_DEBUG("device_create_file vosel_cal fail!\n");
	/**/
