...
#include <../../power/mt6757/bq25896.h>
...
#include "../../misc/mediatek/xhci/tusb320.h"
extern enum typec_current_mode tusb320_current_mode_get(void);
extern enum typec_current_mode wusb3801q_current_mode_get(void);
extern int tusb320_init_done;
extern int wusb3801q_init_done;
//CEI comment end//
...
//CEI comment start: Fix for NONSTANDARD_CHARGER or Type-C//
			...
			if (tusb320_init_done == 1)
				current_mode = tusb320_current_mode_get();
			else if(wusb3801q_init_done == 1)
				current_mode = wusb3801q_current_mode_get();

			battery_log(BAT_LOG_CRTI, "[BATTERY][IUSB] LE(K)=> NONSTANDARD_CHARGER, current_mode = %d, tusb=%d, wusb=%d\n", current_mode, tusb320_init_done, wusb3801q_init_done);
//CEI comment end//
...
//CEI comment start: Fix for NONSTANDARD_CHARGER or Type-C//
			if ( (current_mode == TYPEC_CURRENT_MODE_MID) ||(current_mode == TYPEC_CURRENT_MODE_HIGH) )
			{
...
extern int get_smart_ibat_chg_en(void);
#endif
...
//CEI comments, iusb limit for bcct E
//CEI comment start, Alien battery support
	int alien_current = CHARGE_CURRENT_400_00_MA;
//CEI comment end, Alien battery support
//CEI comment start, SW workaround for IEOC accuracy
	int RT9466_default_IEOC = 150000;
	int RT9466_workaround_IEOC = 200000;
	int ret = 0;
...
//CEI comment start: Fix for NONSTANDARD_CHARGER or Type-C//
				...
				if (BMT_status.charger_type == NONSTANDARD_CHARGER)
				{
					...
					battery_log(BAT_LOG_CRTI, "[BATTERY][IUSB] LE(K)=> NONSTANDARD_CHARGER, current_mode = %d, bcct_level_value=%d, tusb=%d, wusb=%d\n", current_mode, bcct_level_value, tusb320_init_done, wusb3801q_init_done);
				}
...
//CEI comments start, workaround for BQ25896 power drop
	if(bq25896_workaround) {
		if (bq25896_screen_on){ //restore back to previous maximum value when lcd on
			g_temp_input_CC_value = bq25896_pre_inlim;
			battery_log(BAT_LOG_CRTI,"[bq_workaround] bq25896_workaround %d ,  g_temp_input_CC_value %d\n", 
			bq25896_workaround, g_temp_input_CC_value);
		} else { //restore back to workaround 400mA when lcd off
			g_temp_input_CC_value = CHARGE_CURRENT_400_00_MA;
			battery_log(BAT_LOG_CRTI,"[bq_workaround] bq25896_workaround %d ,  g_temp_input_CC_value %d\n", 
			bq25896_workaround, g_temp_input_CC_value);
		} 
	} else {// record input value every cycle before charging termination
		battery_charging_control(CHARGING_CMD_SET_WORKAROUND_TEMP_INPUT_CURRENT,
			&g_temp_input_CC_value);
	}
	
	//There is a case that chargerIC interrupt and workaround flag has been set, but mtk polling hasn't changed to full state yet.
	//In that case, if chargerIC immediately go back to fast charge , flag can't be eliminated in BAT_BatteryFullAction.
       battery_charging_control(CHARGING_CMD_GET_CHARGING_STATUS, &status);
       if (bq25896_workaround && (status == KAL_FALSE)) {
               bq25896_workaround = 0;
               battery_log(BAT_LOG_CRTI, "[bq_workaround] %s() bq25896_workaround =%d \n", __func__, bq25896_workaround);
       }
//CEI comments end, workaround for BQ25896 power drop
...
//Add 500ms thread to prevent backboost
	battery_charging_control(CHARGING_CMD_CHECK_BACKBOOST, NULL);
//CEI comment end
...
//CEI comment, fix screen on too late due to PE detection S
	if(bat_resume_flag==KAL_FALSE) {
		mtk_pep20_check_charger();
		mtk_pep_check_charger();
    	}
//CEI comment, fix screen on too late due to PE detection E
