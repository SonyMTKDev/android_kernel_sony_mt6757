...
static void hall_work_func(struct work_struct *work)
{
	...
	printk("[Keypad] state = %d ,  old_INT_stat = %d \n", (int)state, old_INT_stat);
	...
	if(old_INT_stat != state)
	{
		if(state == 1)
		{
			printk("[Keypad] hall_out  (0 -> 1) OPEN\n");
		}
		else
		{
			printk("[Keypad] hall_out  (1 -> 0) CLOSE\n");
		}
		old_INT_stat = state;
...
int hall_gpio_eint_setup(struct platform_device *pdev)
{
	...
	if (IS_ERR_OR_NULL(pinctrl_hall)) {
		printk("[Keypad] %s , Failed to get pinctrl\n", __FUNCTION__ );
	...
	if (IS_ERR_OR_NULL(pins_hall_default)) {
		printk("[Keypad] %s , Failed to look up cei_hall_out state\n", __FUNCTION__ );
		//goto hall_gpio_pinctrl_err;
	}
#endif
	/* request gpio */
	...
	if (err) {
		printk("[Keypad] %s , unable to request gpio %d\n", __FUNCTION__ , CEI_HALL_OUT);
		goto hall_gpio_pinctrl_err;
	}
#if 0
	/* select pinctrl */
	...
	if (err) {
		printk("[Keypad] %s , Can't select pinctrl default state\n", __FUNCTION__ );
		//return err;
	}
#endif
	/* request irq */
	...
	if (hall_irq_node) {
		...
		printk("[Keypad] %s , hall_irqnr = %d\n", __FUNCTION__ , hall_irqnr );
		if (!hall_irqnr) {
			printk("[Keypad] %s , irq_of_parse_and_map fail!!\n", __FUNCTION__ );
			goto free_gpio;
		}
		...
		if ( request_irq(hall_irqnr, hall_interrupt_handler, irq_flags, "cei_hall_out-eint", NULL) ) {
			printk("[Keypad] %s , IRQ LINE NOT AVAILABLE!!\n", __FUNCTION__ );
			goto free_gpio;
		}
		...
		printk("[Keypad] %s , cannot find cei_hall_out-eint node\n", __FUNCTION__ );
		goto free_gpio;
	}

	printk("[Keypad] %s , exit - Success\n", __FUNCTION__ );
	return 0;

free_gpio:
	gpio_free(CEI_HALL_OUT);
hall_gpio_pinctrl_err:
	printk("[Keypad] %s , exit - Fail\n", __FUNCTION__ );
	return -EINVAL;
}
/**/
/*********************************************************************/
...
{
    printk("[Keypad] Enter kpd_camera_focus_eint_handler\n");
...
{
    printk("[Keypad] Enter kpd_camera_capture_eint_handler\n");
...
static int kpd_camerakey_setup_eint(void)
{
	...
	if (node) {
		...
		enable_irq_wake(capturekey_irq);
	} else {
		kpd_print("[Keypad]%s can't find compatible node\n", __func__);
	}
	return 0;
...
static void camera_focus_eint_func(unsigned long data)
{
	...
	kpd_print("[Keypad]old_state = %d\n",(int)old_state);
       ...
       if(get_cei_customer_project_id() == CUSTOMER_PROJECT_SM21 &&
         (get_cei_hw_id() == CEI_SM21_HW_EVT || get_cei_hw_id() == CEI_SM21_HW_DVT1_1 || get_cei_hw_id() == CEI_SM21_HW_TP))
       {
...
static int kpd_pdrv_probe(struct platform_device *pdev)
{
	...
	if (IS_ERR(pinctrl1)) {
		...
		dev_err(&pdev->dev, "fwq Cannot find vol_down pinctrl1!\n");
		return err;
	}
	pins_default = pinctrl_lookup_state(pinctrl1, "default");
	if (IS_ERR(pins_default)) {
		err = PTR_ERR(pins_default);
		dev_err(&pdev->dev, "fwq Cannot find vol_down pinctrl default!\n");
	}
	pins_eint_int = pinctrl_lookup_state(pinctrl1, "kpd_pins_eint");
	if (IS_ERR(pins_eint_int)) {
		err = PTR_ERR(pins_eint_int);
		dev_err(&pdev->dev, "fwq Cannot find vol_down pinctrl state_eint_int!\n");
		return err;
	}
	...
	if (err!=0) {
		printk("[Keypad] %s , hall_gpio_eint_setup failed (%d)\n", __FUNCTION__ , err );
	}
