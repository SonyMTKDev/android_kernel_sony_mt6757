...
#include <mt-plat/mtk_gpio.h> // cei add
				     ...
				     size_t count, loff_t *ppos)
{
	...
	if ((msg_file->data[0] == 'M') && (msg_file->data[1] == 'G')) {
		...
		if (error)
			pr_debug("tfaContWriteFile error: %d\n", error);
	} else {
		error = dsp_msg(tfa98xx->handle, msg_file->size, msg_file->data);
		if (error)
			pr_debug("dsp_msg error: %d\n", error);
	}
	mutex_unlock(&tfa98xx->dsp_lock);
...
void tfa98xx_shutdown(struct snd_pcm_substream *substream,
						struct snd_soc_dai *dai)
{
	pr_debug("%s() begin \n", __func__);
}
			     ...
			     const struct i2c_device_id *id)
{
	...
	/*mt_set_gpio_mode(115, GPIO_MODE_00);
	mt_set_gpio_dir(115, GPIO_DIR_OUT);
	mt_set_gpio_out(115, GPIO_OUT_ZERO);
	udelay(100);
	pr_err("%s set gpio 115 as low for nxp smart amp tfa9891\n", __FUNCTION__);
       */
	...
	pr_info("tfa98xx.c dev_set_name \n");
