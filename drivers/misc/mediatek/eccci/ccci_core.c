...
static void config_gpio_hotswap(void){
        ...
        CCCI_ERROR_LOG(0, "ccci_core", "config_gpio_hotswap  sim_id = %d\n", cei_sim_id);

        if( cei_sim_id == CEI_SIM_SS){
            // SS
            mt_set_gpio_mode(GPIO46, GPIO_MODE_1);
            mt_set_gpio_pull_select(GPIO46, GPIO_PULL_UP);
            CCCI_ERROR_LOG(0, "ccci_core","[CEI] SS image, set gpio %d mode %d, set pull %d\n", GPIO46, GPIO_MODE_1, GPIO_PULL_UP);
        }else if( cei_sim_id == CEI_SIM_DS ){
            // DS
            mt_set_gpio_mode(GPIO45, GPIO_MODE_1);
            mt_set_gpio_pull_select(GPIO45, GPIO_PULL_UP);
            CCCI_ERROR_LOG(0, "ccci_core","[CEI] SS image, set gpio %d mode %d, set pull %d\n", GPIO45, GPIO_MODE_1, GPIO_PULL_UP);
        }else{
            CCCI_ERROR_LOG(0, "ccci_core","[CEI] Unknown variants\n");
        }
#endif
}
