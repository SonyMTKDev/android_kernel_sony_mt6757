...
#define S24CS64A_DEVICE_ID							0xA2
...
extern int front_inf;
extern int front_macro;
static int front_done=0;
#define IMX234_FRONT_INF_ADDR 0x0D

void read_af_eeprom_IMX234(void)
{
	int i;
	u8 u8data = 0;
	u8 eeprom_data[4]={0};
	u8 puSendCmd2[2]={0};
	u16 eeprom_reg;
	eeprom_reg = IMX234_FRONT_INF_ADDR;

	for(i=0; i<4; i++)
	{
		puSendCmd2[0] = (u8)((eeprom_reg>>8)&0xFF);
		puSendCmd2[1] = (u8)(eeprom_reg&0xFF);
		if (i2c_master_send(g_pstI2Cclient, puSendCmd2, 2) < 0) {
			EEPROMDB("[GT24c32a]read I2C send failed!!\n");
		}

		if (i2c_master_recv(g_pstI2Cclient, &u8data, 1) < 0) {
			EEPROMDB("[GT24c32a]Read I2C AF_EEPROM failed!!\n");
		}
		eeprom_data[i] = u8data;
		eeprom_reg = eeprom_reg + 1;
	}
	front_inf += eeprom_data[1];
	front_inf += eeprom_data[0]*256;
	front_macro += eeprom_data[3];
	front_macro += eeprom_data[2]*256;
}

void read_af_eeprom_GT24c32a(u16 addr, u8 *data, u16 i2c_id, int size)
{
	EEPROMDB("read_af_eeprom_GT24c32a g_camera_sensor_id=0x%x\n", g_camera_sensor_id);
	switch(g_camera_sensor_id)
	{
		case IMX234_SENSOR_ID:
			if (!front_done)
			{
				read_af_eeprom_IMX234();
				EEPROMDB("read_af_eeprom_GT24c32a IMX234_SENSOR_ID already done\n");
				front_done = 1;
			}
			break;
		default:
			EEPROMDB("read_af_eeprom_GT24c32a camera_sensor_id not matched!!\n");
			break;
	}
}
	...
	unsigned char *data, unsigned int size)
{
	...
	read_af_eeprom_GT24c32a(addr, data, g_pstI2Cclient->addr, size);
