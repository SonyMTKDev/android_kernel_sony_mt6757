...
#include <mt-plat/mtk_gpio.h> //BBTEST
...
//SM11L workaround
	...
	#if NFC_DEBUG
	pr_info("%s, IRQ_HANDLED SM11L ! wake lock time out 2sec\n", __func__);
	#endif
		
    //wake_lock_timeout(&pn553_dev->nfc_wake_lock, 10 * HZ);
			       ...
			       size_t count, loff_t *offset)
{
	...
	if (!mt_nfc_get_gpio_value(pn553_dev->irq_gpio)) {
		...
		#if NFC_DEBUG
		dev_info(&pn553_dev->client->dev, "wait_event_interruptible use SM11L: in\n");
		#endif
        //pn553_enable_irq(nfc_irq,pn553_dev);//SM11L workaround
        ret = wait_event_interruptible(pn553_dev->read_wq, do_reading);
        //pn553_disable_irq(nfc_irq,pn553_dev);//SM11L workaround
		#if NFC_DEBUG
...
{
    int gpio_result;
    dev_info(&pn553_dev->client->dev, "%s BBTEST set IRQ GPIO\n", __func__);
    //BBTEST
    //mt_set_gpio_mode(87,0);
    gpio_result = mt_get_gpio_dir(87);
    dev_info(&pn553_dev->client->dev, "gpio_result = %d", gpio_result);

    mt_set_gpio_dir(87, GPIO_DIR_IN);

    gpio_result = mt_get_gpio_dir(87);
    dev_info(&pn553_dev->client->dev, "gpio_result = %d", gpio_result);
}
	//mt_set_gpio_pull_enable(87, GPIO_PULL_ENABLE);
	//mt_set_gpio_pull_select(87, GPIO_PULL_DOWN);
                usleep_range(10000, 10000);
...
static int mt_nfc_get_gpio_value(int gpio_num)
{
	...
	if (mt_nfc_get_gpio_dir(gpio_num) != MTK_NFC_GPIO_DIR_INVALID) {
        ...
        value = mt_get_gpio_in(87 | (0x80000000));
        //pr_info("MTK TEST gpio_get_value %d\n",value);//BBTEST
#else
